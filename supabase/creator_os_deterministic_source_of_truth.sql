...
-- Schema-only migration. Creator-editable content belongs in Creator Admin,
-- not in SQL migrations.
...
alter table public.creators
  ...
  add column if not exists live_button_text text not null default '',
  ...
  add column if not exists theme_preset text not null default '',
  add column if not exists creator_dna text not null default '',
  add column if not exists subscribe_popup_enabled boolean not null default false,
