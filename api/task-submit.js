...
const COMMONPLACE_SITE_CONFIG_WORKSPACE = "commonpl4ce";
...
async function handleBuilderWalletRequest(req, res, input = {}) {
  ...
  if (provider === "google") {
    ...
    if (!googleWalletConfigured()) {
      return send(res, 501, {
        ...
        message: "Wallet delivery is prepared. Google credentials are required before an installable pass can be issued.",
  ...
  if (!signing.configured) {
    ...
    return send(res, 501, {
      ...
      message: "Wallet delivery is prepared. Apple certificates are required before a signed pass can be issued.",
...
function isCommonplaceSiteConfigRequest(req, input = {}) {
  const url = new URL(req.url || "/", `https://${req.headers.host || "doneovernight.com"}`);
  return url.searchParams.get("commonpl4ce_site_config") === "1" ||
    input.action === "commonpl4ce_site_config_publish" ||
    input.source === COMMONPLACE_SITE_CONFIG_SOURCE;
}

async function verifyCommonplaceAdminKey(adminKey) {
...
function normalizeCommonplaceSlot(slot = {}, index = 0, format = "desktop") {
...
function normalizeCommonplaceSiteConfig(input = {}) {
  const source = input && typeof input === "object" ? input : {};
  const hero = source.hero && typeof source.hero === "object" ? source.hero : {};
  const desktop = Array.isArray(hero.desktop) ? hero.desktop.map((slot, index) => normalizeCommonplaceSlot(slot, index, "desktop")).slice(0, 7) : [];
  const mobile = Array.isArray(hero.mobile) ? hero.mobile.map((slot, index) => normalizeCommonplaceSlot(slot, index, "mobile")).slice(0, 7) : [];
  const content = source.content && typeof source.content === "object" ? source.content : {};
  return {
    version: Number(source.version || 1),
    workspace: COMMONPLACE_SITE_CONFIG_WORKSPACE,
...
function validateCommonplaceSiteConfig(config = {}) {
  const errors = [];
  if (config.workspace !== COMMONPLACE_SITE_CONFIG_WORKSPACE) errors.push("workspace_invalid");
...
async function latestCommonplaceSiteConfig() {
  const rows = await supabaseFetch([
  ...
  const config = raw.config && typeof raw.config === "object" ? normalizeCommonplaceSiteConfig(raw.config) : null;
  return config ? {
    config,
...
async function handleCommonplaceSiteConfigGet(req, res) {
  void req;
  try {
    const published = await latestCommonplaceSiteConfig();
    if (!published) {
...
async function handleCommonplaceSiteConfigPost(req, res, input = {}) {
  ...
  const authorized = await verifyCommonplaceAdminKey(adminKey);
  if (!authorized) {
  ...
  const config = normalizeCommonplaceSiteConfig({
    ...(input.config || {}),
    version: Number(input.config?.version || 1) + 1,
    updatedAt: now
  });
  const errors = validateCommonplaceSiteConfig(config);
  if (errors.length) {
    return send(res, 400, { success: false, error: "Config is not publishable", code: "CONFIG_NOT_PUBLISHABLE", errors });
  }
  ...
  return send(res, 200, {
    ...
    config,
    publishedAt: now,
    taskId: clean(record?.task_id || taskId)
  });
}
