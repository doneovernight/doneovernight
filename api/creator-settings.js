...
function getSupabaseConfig() {
  ...
  if (!url || !serviceRoleKey) {
    const error = new Error("Creator settings are not configured");
    error.code = "CREATOR_SETTINGS_NOT_CONFIGURED";
...
async function supabaseFetch(pathname, options = {}) {
  ...
  try {
    ...
    if (!response.ok) {
      const error = new Error("Supabase creator settings request failed: " + response.status);
...
async function fetchCreator(slug = "mina") {
  ...
  const rows = await supabaseFetch("creators?slug=eq." + safeSlug + "&select=" + PUBLIC_FIELDS + "&limit=1");
...
async function saveCreator(input = {}) {
  const rows = await supabaseFetch("creators?on_conflict=id&select=" + PUBLIC_FIELDS, {
    method: "POST",
...
module.exports = async function handler(req, res) {
  try {
    if (req.method === "GET") {
      try {
        const slug = clean(req.query && req.query.slug) || "mina";
