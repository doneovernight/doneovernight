...
const CreatorCapabilities = require("../lib/creator-capabilities");
...
const CREATOR_FIELDS_WITHOUT_CAPABILITY_JSON = BASE_CREATOR_FIELDS.concat(AMBIENT_CREATOR_FIELDS, PHASE_1_4_CREATOR_FIELDS, PHASE_2_CREATOR_FIELDS, CAPABILITY_CREATOR_FIELDS.filter((field) => field !== "capabilities"), PHASE_3_CREATOR_FIELDS, POLL_CREATOR_FIELDS, LINK_BLOCK_CREATOR_FIELDS, INTRO_AUDIO_CREATOR_FIELDS, TIKTOK_WELCOME_CREATOR_FIELDS).join(",");
...
function defaultCreatorForSlug(value = DEFAULT_CREATOR_SLUG) {
  ...
  return {
    ...
    live_url: "",
    live_status: false,
    capabilities: CreatorCapabilities.normalizeCapabilities({ creator_dna: "editorial" }),
    live_enabled: false,
...
function creatorDefaultText(value, key, defaults = DEFAULT_MINA_SETTINGS) {
  return creatorGateText(value, DEFAULT_MINA_SETTINGS[key], defaults[key]) || clean(defaults[key]);
}

function capabilityDefaultsForDna(value) {
  return CreatorCapabilities.mirrorLegacyFields({ creator_dna: normalizeCreatorDna(value) });
}
...
function normalizeCreatorDna(value) {
  const allowed = new Set(["artist", "streamer", "influencer", "founder", "editorial", "fitness", "musician", "podcaster", "gamer", "realtor", "coach", "restaurant", "business"]);
...
function mediaNameExtension(fallbackName = "") {
  const ext = clean(fallbackName).toLowerCase().match(/\.([a-z0-9]{2,5})$/);
  return ext ? ext[1] : "";
}

function isProfilePhotoUpload(mimeType, fallbackName = "") {
  const named = mediaNameExtension(fallbackName);
  const normalized = clean(mimeType).toLowerCase();
  return ["png", "jpg", "jpeg"].includes(named) &&
    (!normalized || ["image/png", "image/jpeg"].includes(normalized) || GENERIC_UPLOAD_MIME_TYPES.has(normalized));
}
...
function normalizeProfilePhotoMimeType(mimeType, fallbackName = "") {
  const named = mediaNameExtension(fallbackName);
  const normalized = clean(mimeType).toLowerCase();
  if (!["png", "jpg", "jpeg"].includes(named)) return "";
  if (normalized && !["image/png", "image/jpeg"].includes(normalized) && !GENERIC_UPLOAD_MIME_TYPES.has(normalized)) return "";
...
function normalizeCreator(row = {}) {
  ...
  const capabilities = CreatorCapabilities.normalizeCapabilities({
    ...capabilityDefaults,
    ...defaults,
    ...row,
    creator_dna: dna
  ...
  return {
    ...
    discord_url: creatorDefaultText(row.discord_url, "discord_url", defaults),
    instagram_url: clean(row.instagram_url),
    tiktok_coins_url: creatorDefaultText(row.tiktok_coins_url, "tiktok_coins_url", defaults),
    ...
    discord_link_title: creatorDefaultText(row.discord_link_title, "discord_link_title", defaults),
    discord_link_subtitle: creatorDefaultText(row.discord_link_subtitle, "discord_link_subtitle", defaults),
    discord_link_cta_label: creatorDefaultText(row.discord_link_cta_label, "discord_link_cta_label", defaults),
    tiktok_link_visible: bool(row.tiktok_link_visible, true),
    tiktok_link_title: creatorDefaultText(row.tiktok_link_title, "tiktok_link_title", defaults),
    tiktok_link_subtitle: creatorDefaultText(row.tiktok_link_subtitle, "tiktok_link_subtitle", defaults),
    tiktok_link_cta_label: creatorDefaultText(row.tiktok_link_cta_label, "tiktok_link_cta_label", defaults),
    battle_link_visible: bool(row.battle_link_visible, true),
    battle_link_title: creatorDefaultText(row.battle_link_title, "battle_link_title", defaults),
    battle_link_subtitle: creatorDefaultText(row.battle_link_subtitle, "battle_link_subtitle", defaults),
    battle_link_cta_label: creatorDefaultText(row.battle_link_cta_label, "battle_link_cta_label", defaults),
    support_link_visible: bool(row.support_link_visible, false),
    support_link_title: creatorDefaultText(row.support_link_title, "support_link_title", defaults),
    support_link_subtitle: creatorDefaultText(row.support_link_subtitle, "support_link_subtitle", defaults),
    support_link_cta_label: creatorDefaultText(row.support_link_cta_label, "support_link_cta_label", defaults),
    ...
    business_link_title: creatorDefaultText(row.business_link_title, "business_link_title", defaults),
    business_link_subtitle: creatorDefaultText(row.business_link_subtitle, "business_link_subtitle", defaults),
    business_link_cta_label: creatorDefaultText(row.business_link_cta_label, "business_link_cta_label", defaults),
    music_link_visible: bool(row.music_link_visible, false),
    music_link_title: creatorDefaultText(row.music_link_title, "music_link_title", defaults),
    music_link_subtitle: creatorDefaultText(row.music_link_subtitle, "music_link_subtitle", defaults),
    music_link_cta_label: creatorDefaultText(row.music_link_cta_label, "music_link_cta_label", defaults),
    newsletter_cta_label: creatorDefaultText(row.newsletter_cta_label, "newsletter_cta_label", defaults),
    newsletter_destination: clean(row.newsletter_destination),
    faq_link_visible: bool(row.faq_link_visible, false),
    faq_link_title: creatorDefaultText(row.faq_link_title, "faq_link_title", defaults),
    faq_link_subtitle: creatorDefaultText(row.faq_link_subtitle, "faq_link_subtitle", defaults),
    faq_link_cta_label: creatorDefaultText(row.faq_link_cta_label, "faq_link_cta_label", defaults),
    ...
    community_link_title: creatorDefaultText(row.community_link_title, "community_link_title", defaults),
    community_link_subtitle: creatorDefaultText(row.community_link_subtitle, "community_link_subtitle", defaults),
    community_link_cta_label: creatorDefaultText(row.community_link_cta_label, "community_link_cta_label", defaults),
    community_link_url: creatorDefaultText(row.community_link_url, "community_link_url", defaults) ||
      creatorDefaultText(row.discord_invite_url, "discord_invite_url", defaults) ||
      creatorDefaultText(row.discord_url, "discord_url", defaults),
    ...
    discord_invite_url: creatorDefaultText(row.discord_invite_url, "discord_invite_url", defaults) ||
      creatorDefaultText(row.discord_url, "discord_url", defaults),
...
async function fetchCreatorFromTable(slug = DEFAULT_CREATOR_SLUG) {
  ...
  const selectAttempts = [
    CREATOR_FIELDS,
    CREATOR_FIELDS_WITHOUT_CAPABILITY_JSON,
...
function creatorPayload(input = {}) {
  ...
  return {
    ...
    discord_url: creatorDefaultText(input.discord_url, "discord_url", defaults),
    instagram_url: clean(input.instagram_url),
    tiktok_coins_url: creatorDefaultText(input.tiktok_coins_url, "tiktok_coins_url", defaults),
    ...
    discord_link_title: creatorDefaultText(input.discord_link_title, "discord_link_title", defaults),
    discord_link_subtitle: creatorDefaultText(input.discord_link_subtitle, "discord_link_subtitle", defaults),
    discord_link_cta_label: creatorDefaultText(input.discord_link_cta_label, "discord_link_cta_label", defaults),
    tiktok_link_visible: bool(input.tiktok_link_visible, true),
    tiktok_link_title: creatorDefaultText(input.tiktok_link_title, "tiktok_link_title", defaults),
    tiktok_link_subtitle: creatorDefaultText(input.tiktok_link_subtitle, "tiktok_link_subtitle", defaults),
    tiktok_link_cta_label: creatorDefaultText(input.tiktok_link_cta_label, "tiktok_link_cta_label", defaults),
    battle_link_visible: bool(input.battle_link_visible, true),
    battle_link_title: creatorDefaultText(input.battle_link_title, "battle_link_title", defaults),
    battle_link_subtitle: creatorDefaultText(input.battle_link_subtitle, "battle_link_subtitle", defaults),
    battle_link_cta_label: creatorDefaultText(input.battle_link_cta_label, "battle_link_cta_label", defaults),
    support_link_visible: bool(input.support_link_visible, false),
    support_link_title: creatorDefaultText(input.support_link_title, "support_link_title", defaults),
    support_link_subtitle: creatorDefaultText(input.support_link_subtitle, "support_link_subtitle", defaults),
    support_link_cta_label: creatorDefaultText(input.support_link_cta_label, "support_link_cta_label", defaults),
    ...
    business_link_title: creatorDefaultText(input.business_link_title, "business_link_title", defaults),
    business_link_subtitle: creatorDefaultText(input.business_link_subtitle, "business_link_subtitle", defaults),
    business_link_cta_label: creatorDefaultText(input.business_link_cta_label, "business_link_cta_label", defaults),
    music_link_visible: bool(input.music_link_visible, false),
    music_link_title: creatorDefaultText(input.music_link_title, "music_link_title", defaults),
    music_link_subtitle: creatorDefaultText(input.music_link_subtitle, "music_link_subtitle", defaults),
    music_link_cta_label: creatorDefaultText(input.music_link_cta_label, "music_link_cta_label", defaults),
    newsletter_cta_label: creatorDefaultText(input.newsletter_cta_label, "newsletter_cta_label", defaults),
    newsletter_destination: clean(input.newsletter_destination),
    faq_link_visible: bool(input.faq_link_visible, false),
    faq_link_title: creatorDefaultText(input.faq_link_title, "faq_link_title", defaults),
    faq_link_subtitle: creatorDefaultText(input.faq_link_subtitle, "faq_link_subtitle", defaults),
    faq_link_cta_label: creatorDefaultText(input.faq_link_cta_label, "faq_link_cta_label", defaults),
    ...
    community_link_title: creatorDefaultText(input.community_link_title, "community_link_title", defaults),
    community_link_subtitle: creatorDefaultText(input.community_link_subtitle, "community_link_subtitle", defaults),
    community_link_cta_label: creatorDefaultText(input.community_link_cta_label, "community_link_cta_label", defaults),
    community_link_url: creatorDefaultText(input.community_link_url, "community_link_url", defaults) ||
      creatorDefaultText(input.discord_invite_url, "discord_invite_url", defaults) ||
      creatorDefaultText(input.discord_url, "discord_url", defaults),
    ...
    discord_invite_url: creatorDefaultText(input.discord_invite_url, "discord_invite_url", defaults) ||
      creatorDefaultText(input.discord_url, "discord_url", defaults),
...
module.exports = async function handler(req, res) {
  ...
  if (query.creator_health === "1") {
    try {
      return await handleCreatorHealth(req, res);
    } catch (error) {
      return send(res, error.statusCode || 500, {
        success: false,
