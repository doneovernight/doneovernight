...
function getSupabaseConfig() {
  ...
  if (!url || !serviceRoleKey) {
    const error = new Error("Admin clients are not configured");
    error.code = "ADMIN_CLIENTS_NOT_CONFIGURED";
...
async function fetchClients() {
  ...
  try {
    const response = await fetch(`${url}/rest/v1/portal_requests?select=*&order=created_at.desc`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      const error = new Error(`Supabase clients read failed: ${response.status}`);
