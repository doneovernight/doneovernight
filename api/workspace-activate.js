...
const { sendWorkspaceActivationEmail } = require("../lib/email/workspace-activation-email");
...
function upsertProject(rawPayload = {}, project = {}) {
  ...
  const key = clean(project.task_id);
  const nextProjects = [
    project,
    ...projects.filter((item) => clean(item.task_id) !== key)
  ].slice(0, 50);
  return {
    ...current,
...
async function createWorkspaceActivityEvents({ task, workspaceSlug, email, paymentReference, amountPaid }) {
  const taskId = clean(task.task_id || task.taskId || task.id);
  const events = [
    {
      title: "Payment received",
      message: `Payment received for ${taskId}.`,
...
async function activateWorkspace(req, input = {}) {
  ...
  const activationEmail = await sendWorkspaceActivationEmail(updatedTask, {
    email,
    ...
    reason: "failed",
    provider: "none",
    error: error.code || "WORKSPACE_ACTIVATION_EMAIL_FAILED"
  }));
