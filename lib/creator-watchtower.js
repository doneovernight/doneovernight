...
const CRITICAL_ALERT_WINDOW_MS = TELEGRAM_DEDUPE_MIN_MS;
...
function alertWindowFor(alert = {}) {
  const windowMs = alert.severity === "critical" ? CRITICAL_ALERT_WINDOW_MS : ALERT_WINDOW_MS;
  return Math.max(windowMs, TELEGRAM_DEDUPE_MIN_MS);
}
...
function telegramGateFor(alert = {}) {
  ...
  if (isSuppressedAdminAuthNoise(alert)) {
    return { enabled: false, reason: "admin_creator_settings_401_auth_noise" };
  }
...
async function reportCreatorError(input = {}) {
  ...
  if (!telegramGate.enabled) {
    ...
    return { success: true, alert_sent: false, severity: alert.severity, suppressed: true, reason: telegramGate.reason, telegram_enabled: false, dedupe_key: alert.dedupe_key, fingerprint: alert.fingerprint };
  }
  ...
  if (rateLimited) {
    ...
    return { success: true, alert_sent: false, severity: alert.severity, rate_limited: true, telegram_enabled: true, dedupe_key: alert.dedupe_key, fingerprint: alert.fingerprint, dedupe_window_ms: dedupeWindowMs };
  }
