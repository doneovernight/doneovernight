...
const { creatorOsEnvironment, getSupabaseRuntimeConfig } = require("./creator-os-environment");
...
async function checkStorageBucket() {
  ...
  try {
    ...
    const response = await fetch(url + "/storage/v1/bucket/" + encodeURIComponent(CREATOR_MEDIA_BUCKET), {
      headers: {
...
async function checkPublicPage(req, slug, creator = {}) {
  try {
    ...
    const ok = response.ok && text.includes(displayName) && text.includes(introKey);
    return {
      status: ok ? "healthy" : "degraded",
      code: response.status,
      checks: {
        loads: response.ok,
        containsCreator: text.includes(displayName),
        containsIntro: text.includes(introKey)
      }
    };
  } catch (error) {
