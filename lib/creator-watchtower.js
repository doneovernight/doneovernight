...
function isPreviewFetchNoise(alert = {}) {
  const area = String(alert.area || "").toLowerCase();
  const action = String(alert.action || "").toLowerCase();
  const error = String(alert.error || alert.message || "").toLowerCase();
  const url = String(alert.url || "").toLowerCase();
  const isPreviewUrl = url.includes("?preview=1") || url.includes("&preview=1");
  return isPreviewUrl &&
    area === "public" &&
    action.startsWith("fetch") &&
    (error.includes("failed to fetch") || action.includes("failed"));
}

function telegramGateFor(alert = {}) {
  const severity = normalizeSeverity(alert.severity, "error");
  if (isPreviewFetchNoise(alert)) {
    return { enabled: false, reason: "preview_fetch_noise" };
  }
