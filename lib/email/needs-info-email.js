...
function buildWorkspaceAddInformationUrl(task = {}, reference = "") {
  ...
  const baseUrl = workspaceSlug
    ? `https://portal.doneovernight.com/@${encodeURIComponent(workspaceSlug)}`
    : workspaceUrl || "";
...
function buildNeedsInfoEmail(task = {}) {
  ...
  const operatorContext = task.raw_payload?.requested_by_operator && typeof task.raw_payload.requested_by_operator === "object"
    ? task.raw_payload.requested_by_operator
    : null;
  const operatorLabel = operatorContext
    ? [
        clean(operatorContext.display_name || operatorContext.name),
        clean(operatorContext.slug || operatorContext.handle) ? `@${clean(operatorContext.slug || operatorContext.handle).replace(/^@+/, "")}` : ""
      ].filter(Boolean).join(" · ")
    : "";
  ...
  const email = buildClientEmail({
    ...
    rows: [
      ...
      ...(operatorLabel ? [[language === "nl" ? "Operator" : "Operator", operatorLabel]] : [])
    ],
    infoCards: [
...
function buildNeedsInfoEmailWebhookPayload(task = {}) {
  ...
  return {
    ...
    operator_display_name: clean(task.raw_payload?.requested_by_operator?.display_name || task.raw_payload?.requested_by_operator?.name || ""),
    operator_slug: clean(task.raw_payload?.requested_by_operator?.slug || task.raw_payload?.requested_by_operator?.handle || ""),
