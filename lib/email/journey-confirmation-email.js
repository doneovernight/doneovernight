...
function buildJourneyConfirmationEmail(input = {}) {
  ...
  const subject = clean(input.subject) || (isDutch ? "Je zit in het systeem." : "You're in the system.");
  const preheader = isDutch
    ? "Welkom bij DONEOVERNIGHT. Je journey is gestart."
    : "Welcome to DONEOVERNIGHT. Your journey officially started.";
  ...
  const labels = isDutch ? {
    unlocked: "Toegang ontgrendeld",
    headline: "Je zit erin.",
    welcome: "Welkom bij DONEOVERNIGHT.",
    started: "Je journey is gestart.",
    accessIntro: "Je hebt toegang gekregen tot",
    publicBuild: "Dit is waar we bouwen in het openbaar.",
    access: ["Live Builds", "Viewer Builds", "Resources", "Build Journal", "Future releases"],
    primary: "Ga verder met je journey",
    live: "Bekijk Live Builds",
    resources: "Open Resources",
    viewerBuilds: "Verstuur een build idee",
    follow: "Volg de journey",
    early: "Je bent een van de early builders.",
    signoff: "Tot overnight.",
    ...
    headline: "You're in.",
    welcome: "Welcome to DONEOVERNIGHT.",
    started: "Your journey officially started.",
    accessIntro: "You now have access to",
    publicBuild: "Everything is built in public.",
    access: ["Live Builds", "Viewer Builds", "Resources", "Build Journal", "Future releases"],
    primary: "Continue your journey",
    live: "View Live Builds",
    resources: "Resources",
    viewerBuilds: "Viewer Builds",
    follow: "Follow the Journey",
    early: "You're one of the early builders.",
    signoff: "See you overnight.",
    journeyId: "Journey ID",
    builderType: "Builder Type",
    interests: "Chosen interests",
    result: "Result",
  ...
  const secondaryLinks = [
    [labels.live, liveUrl],
    [labels.resources, resourcesUrl],
    [labels.viewerBuilds, viewerBuildsUrl],
    [labels.follow, TIKTOK_URL]
  ];
  const metaRows = [
    journeyId ? [labels.journeyId, journeyId] : null,
    chosenPath ? [labels.builderType, chosenPath] : null,
    chosenInterests.length ? [labels.interests, chosenInterests.join(", ")] : null,
    result ? [labels.result, result] : null,
    language.email_language ? [labels.selectedLanguage, language.email_language.toUpperCase()] : null
  ].filter(Boolean);
  ...
  const accessHtml = labels.access.map((item) => `
                    <tr>
              ...
              <table width="640" cellpadding="0" cellspacing="0" border="0" style="width:100%;max-width:640px;background:#08090b;border:1px solid rgba(245,241,234,0.1);border-radius:28px;overflow:hidden;">
                ...
                <tr>
                  <td style="padding:18px 46px 34px;">
                    <div style="color:#d9bd83;font-size:11px;letter-spacing:4px;text-transform:uppercase;margin-bottom:18px;">${escapeHtml(labels.unlocked)}</div>
                    <h1 style="margin:0;color:#f5f1ea;font-family:Georgia,'Times New Roman',serif;font-size:64px;line-height:0.9;font-weight:400;letter-spacing:-1px;">${escapeHtml(labels.headline)}</h1>
                    <p style="margin:28px 0 0;color:#aaa39a;font-size:17px;line-height:1.8;">${escapeHtml(labels.welcome)}<br>${escapeHtml(labels.started)}</p>
                  </td>
                </tr>
                <tr>
                  <td style="padding:0 46px 32px;">
                    <div style="background:rgba(245,241,234,0.03);border:1px solid rgba(245,241,234,0.09);border-radius:22px;padding:24px;">
                      <div style="color:#77716a;font-size:10px;letter-spacing:3px;text-transform:uppercase;margin-bottom:12px;">${escapeHtml(labels.accessIntro)}</div>
                      <table width="100%" cellpadding="0" cellspacing="0" border="0">${accessHtml}</table>
                    </div>
                    <p style="margin:24px 0 0;color:#aaa39a;font-size:15px;line-height:1.7;">${escapeHtml(labels.publicBuild)}</p>
                  </td>
                </tr>
                ${metaHtml}
                <tr>
                ...
                <tr>
                  <td style="border-top:1px solid rgba(245,241,234,0.08);padding:28px 46px 38px;color:#77716a;font-size:12px;line-height:2;letter-spacing:2px;text-transform:uppercase;">
                    ${escapeHtml(labels.early)}<br>
                    ${escapeHtml(labels.signoff)}
                  </td>
                </tr>
              </table>
            </td>
