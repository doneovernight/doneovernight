...
function firstTokenizedReviewUrl(...values) {
  return values.map(clean).find(hasReviewToken) || "";
}
...
function buildQuoteEmail(task = {}) {
  ...
  const existingReviewUrl = firstTokenizedReviewUrl(
  ...
  const reviewUrl = existingReviewUrl || buildSecureReviewUrl(task);
  ...
  const email = buildClientEmail({
    ...
    intro: `Hi ${name}, your execution plan is ready to review. Open your secure review page to confirm the scope, timing, and start.`,
    rows: [
      ["Reference", reference],
    ...
    body: [
      "Review the execution plan before starting. The secure review page contains the scope, timing, investment, and Approve & Start step.",
      quoteNote ? `Scope note: ${quoteNote}` : ""
    ],
