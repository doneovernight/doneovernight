...
function buildQuoteEmail(task = {}) {
  ...
  const text = [
    ...
    quoteAmount ? `Investment: ${quoteAmount}` : null,
    "",
    primaryCtaUrl ? `Approve & Start: ${primaryCtaUrl}` : null,
    paymentLink ? `Start link: ${paymentLink}` : null,
    "",
      ...
      <body style="margin:0;padding:0;background:#050608;color:#f5f1ea;font-family:Inter,Arial,sans-serif">
        ...
        <main style="max-width:560px;margin:0 auto;padding:44px 24px">
          <section style="border:1px solid rgba(233,196,138,.22);border-radius:8px;background:rgba(245,241,234,.035);padding:32px 28px">
            ...
            <h1 style="margin:0 0 18px;color:#f5f1ea;font-family:Georgia,'Times New Roman',serif;font-size:32px;line-height:1.12;font-weight:400">Execution plan ready.</h1>
            <p style="margin:0 0 22px;color:rgba(245,241,234,.78);font-size:15px;line-height:1.7">Hi ${escapeHtml(name)}, your DONEOVERNIGHT execution plan is ready for approval. Once approved, operators are assigned and execution begins.</p>
            ...
            ${primaryCtaUrl ? `<p style="margin:0 0 14px"><a href="${escapeHtml(primaryCtaUrl)}" style="display:inline-block;padding:13px 18px;border:1px solid rgba(233,196,138,.44);border-radius:999px;background:rgba(233,196,138,.08);color:#e9c48a;text-decoration:none;font-size:11px;font-weight:800;letter-spacing:.14em;text-transform:uppercase">Approve &amp; Start</a></p>` : ""}
            ${paymentLink ? `<p style="margin:0 0 24px;color:rgba(245,241,234,.7);font-size:13px;line-height:1.7">Start link fallback:<br><a href="${escapeHtml(paymentLink)}" style="color:#e9c48a;text-decoration:underline;text-underline-offset:4px">${escapeHtml(paymentLink)}</a></p>` : ""}
            <p style="margin:0;color:rgba(245,241,234,.58);font-size:13px;line-height:1.7">Operators are assigned after approval. Workspace access opens once execution is released.</p>
          </section>
