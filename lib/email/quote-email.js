...
function buildQuoteEmail(task = {}) {
  ...
  const copy = language === "nl"
    ? {
        subject: "DONEOVERNIGHT — Uitvoeringsplan klaar",
        preheader: `Uitvoeringsplan klaar. Referentie: ${reference}`,
        statusLabel: "UITVOERINGSPLAN KLAAR",
        title: "Uitvoeringsplan klaar.",
        intro: "Je uitvoeringsplan is klaar.",
        lead: "Open je veilige reviewpagina om verder te gaan.",
        ctaLabel: "Review uitvoeringsplan"
      }
    : {
        subject: "DONEOVERNIGHT — Execution plan ready",
  ...
  const email = buildClientEmail({
    ...
    ctaLabel: primaryCtaUrl ? copy.ctaLabel : "",
...
function buildQuoteEmailWebhookPayload(task = {}) {
  ...
  return {
    ...
    cta_label: email.language === "nl" ? "Review uitvoeringsplan" : "Review Execution Plan",
