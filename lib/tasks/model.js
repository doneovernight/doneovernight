...
function buildTaskPayload(input, taskId, now = new Date()) {
  ...
  const budget = clean(input.client_budget || input.clientBudget || input.budget || input.estimatedBudget || input.estimated_budget || input.project_budget || input.projectBudget);
