...
function getConfig(overrides = {}) {
  ...
  return {
    ...
    autonomy: {
      ...
      minimumDailyTarget: 2,
      preferredDailyRange: [3, 4],
      dailyCap: number(overrides.autonomyDailyCap || process.env.CONTENT_AUTONOMY_DAILY_CAP, 5, 1, 5),
      weeklyCap: number(overrides.autonomyWeeklyCap || process.env.CONTENT_AUTONOMY_WEEKLY_CAP, 20, 1, 20),
      minimumIntervalMinutes: number(overrides.autonomyMinimumIntervalMinutes || process.env.CONTENT_AUTONOMY_MIN_INTERVAL_MINUTES, 180, 180, 1440),
      scheduleLeadMinutes: number(overrides.autonomyScheduleLeadMinutes || process.env.CONTENT_AUTONOMY_SCHEDULE_LEAD_MINUTES, 5, 1, 15),
      topicCooldownHours: number(overrides.autonomyTopicCooldownHours || process.env.CONTENT_AUTONOMY_TOPIC_COOLDOWN_HOURS, 18, 18, 168),
      sourceLimitHours: number(overrides.autonomySourceLimitHours || process.env.CONTENT_AUTONOMY_SOURCE_LIMIT_HOURS || process.env.CONTENT_AUTONOMY_SOURCE_LIMIT_48H, 24, 24, 168),
      sourceLimit: number(overrides.autonomySourceLimit || process.env.CONTENT_AUTONOMY_SOURCE_LIMIT, 2, 1, 2),
      ...
      windows: clean(overrides.autonomyWindows || process.env.CONTENT_AUTONOMY_WINDOWS) || "08:00-22:00",
      thresholds: {
