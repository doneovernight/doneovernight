...
async function collectAnalytics(options = {}) {
  ...
  for (const row of originals) {
    ...
    const postClassification = row.classification || (row.publication_origin === "agent" ? "agent_original" : "manual_original"); const publicationOrigin = row.publication_origin || (publication ? "agent" : "manual");
    await repo.createAnalytics({ publication_id: publication?.id || null, account_activity_x_post_id: row.x_post_id, x_post_id: row.x_post_id, snapshot_key: snapshotKey(now), recorded_at: new Date(now).toISOString(), views: publicMetrics.impression_count ?? null, likes: publicMetrics.like_count ?? null, replies: publicMetrics.reply_count ?? null, quotes: publicMetrics.quote_count ?? null, reposts: publicMetrics.retweet_count ?? null, bookmarks: publicMetrics.bookmark_count ?? null, profile_visits: privateMetrics.user_profile_clicks ?? null, follower_count: Number.isFinite(followers) ? followers : null, followers_gained_after_post: gained, raw_metrics: { classification: postClassification, publication_origin: publicationOrigin, ingestion_source: row.ingestion_source || null, public: publicMetrics, private: privateMetrics } });
    if (repo.savePerformanceMemory) await repo.savePerformanceMemory({ publication_id: publication?.id || null, account_activity_x_post_id: row.x_post_id, x_post_id: row.x_post_id, draft_id: publication?.draft_id || null, topic: publication?.topic_cluster || null, views: publicMetrics.impression_count ?? null, likes: publicMetrics.like_count ?? null, replies: publicMetrics.reply_count ?? null, quotes: publicMetrics.quote_count ?? null, reposts: publicMetrics.retweet_count ?? null, bookmarks: publicMetrics.bookmark_count ?? null, normalized_performance: Number(publicMetrics.impression_count || 0) ? (Number(publicMetrics.like_count || 0) + Number(publicMetrics.reply_count || 0) + Number(publicMetrics.quote_count || 0) + Number(publicMetrics.retweet_count || 0) + Number(publicMetrics.bookmark_count || 0)) / Number(publicMetrics.impression_count) : null, final_score: null, metrics: { classification: postClassification, publication_origin: publicationOrigin, ingestion_source: row.ingestion_source || null, public: publicMetrics, private: privateMetrics }, recorded_at: new Date(now).toISOString(), updated_at: new Date(now).toISOString() });
    result.snapshots += 1; if (publicationOrigin === "agent") result.agent_posts += 1; else if (publicationOrigin === "manual") result.manual_posts += 1;
  }
  return result;
}
