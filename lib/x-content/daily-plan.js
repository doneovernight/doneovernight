...
function atLocal(day, minutes, timezone) {
  ...
  const probe = localParts(approx, timezone); const desired = minutes; const actual = Number(probe.hour) * 60 + Number(probe.minute); return new Date(approx.getTime() + (desired - actual) * 60_000);
}

function planSlots({ now = Date.now(), timezone = "Europe/Amsterdam", count = 3 } = {}) {
  const target = Math.min(MAX_POSTS, Math.max(MIN_POSTS, Number(count) || MIN_POSTS)); const current = new Date(now); const days = [dayKey(current, timezone), dayKey(new Date(current.getTime() + 86_400_000), timezone)]; const baseMinutes = [9 * 60 + 5, 12 * 60 + 25, 16 * 60 + 10, 19 * 60 + 20, 21 * 60 + 5];
  let selected = [];
  for (const day of days) {
    selected = baseMinutes.map((minutes, index) => atLocal(day, minutes + offsetFor(day, index), timezone)).filter((date) => date.getTime() >= now + 5 * 60_000 && localParts(date, timezone).hour >= "08" && localParts(date, timezone).hour < "22");
    if (selected.length >= target || day !== days[0]) break;
  }
  selected = selected.slice(0, target); const slots = selected.map((date, index) => ({ index, planned_for: date.toISOString(), date_key: dayKey(date, timezone), objective: MIX[index % MIX.length].objective, content_pillar: MIX[index % MIX.length].pillar, discovery_tiers: MIX[index % MIX.length].tiers, fallback_tier: MIX[index % MIX.length].fallback, expected_audience_value: "A concrete, source-backed operating takeaway for builders, founders, or operators." }));
  return { target: { minimum: MIN_POSTS, preferred: PREFERRED_RANGE, maximum: MAX_POSTS }, timezone, window: WINDOW, minimum_spacing_minutes: MIN_SPACING_MINUTES, slots };
}
...
module.exports = { MIN_POSTS, PREFERRED_RANGE, MAX_POSTS, MIN_SPACING_MINUTES, MIX, CURATED_PRINCIPLES, planSlots, respectsSpacing, remainingMinimum, dailyStatus, dayKey };
