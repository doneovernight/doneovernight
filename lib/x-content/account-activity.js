...
function origin(post, agentPostIds) { if (isRepost(post)) return "repost"; if (isReply(post)) return "reply"; return agentPostIds.has(String(post.id)) ? "agent_original" : "manual_original"; }
function activityRow(post, { accountId, agentPostIds, existing, now }) {
  const publicationOrigin = origin(post, agentPostIds);
  return { x_post_id: String(post.id), account_id: String(accountId), text: String(post.text || ""), created_at: post.created_at || new Date(now).toISOString(), publication_origin: publicationOrigin, source_kind: "authenticated_timeline", is_reply: publicationOrigin === "reply", is_repost: publicationOrigin === "repost", is_currently_visible: true, discovered_at: existing?.discovered_at || new Date(now).toISOString(), last_seen_at: new Date(now).toISOString() };
}
...
async function recordAgentPublication({ xPostId, accountId, text, publishedAt = new Date().toISOString() }, options = {}) {
  ...
  const row = { x_post_id: String(xPostId), account_id: String(accountId), text: String(text || ""), created_at: publishedAt, publication_origin: "agent_original", source_kind: "agent_publication", is_reply: false, is_repost: false, is_currently_visible: true, discovered_at: new Date(now).toISOString(), last_seen_at: new Date(now).toISOString() };
  await repo.upsertAccountActivity([row]);
  return row;
}

module.exports = { MAX_PAGES, PAGE_SIZE, SYNC_SETTING, TIMEZONE, activityRow, activitySummary, isReply, isRepost, localDay, recordAgentPublication, syncAccountActivity };
