...
function origin(post, agentPostIds) { if (isRepost(post)) return "repost"; if (isReply(post)) return "reply"; return agentPostIds.has(String(post.id)) ? "agent" : "manual"; }
function activityRow(post, { accountId, agentPostIds, existing, now }) {
  const publicationOrigin = origin(post, agentPostIds);
  return { x_post_id: String(post.id), account_id: String(accountId), text: String(post.text || ""), created_at: post.created_at || new Date(now).toISOString(), publication_origin: publicationOrigin, is_reply: publicationOrigin === "reply", is_repost: publicationOrigin === "repost", is_currently_visible: true, discovered_at: existing?.discovered_at || new Date(now).toISOString(), last_seen_at: new Date(now).toISOString() };
}
function activitySummary(records = [], { now = Date.now(), timeZone = TIMEZONE, status = {} } = {}) {
  const current = records.filter((record) => record?.is_currently_visible !== false); const today = localDay(now, timeZone); const original = current.filter((record) => !record.is_reply && !record.is_repost); const todayOriginal = original.filter((record) => localDay(record.created_at, timeZone) === today); const agentToday = todayOriginal.filter((record) => record.publication_origin === "agent").length;
  return { posts_today: todayOriginal.length, known_total_posts: original.length, agent_published_today: agentToday, manual_posts_today: Math.max(0, todayOriginal.length - agentToday), replies_today: current.filter((record) => record.is_reply && localDay(record.created_at, timeZone) === today).length, reposts_today: current.filter((record) => record.is_repost && localDay(record.created_at, timeZone) === today).length, last_x_sync: status.last_success_at || status.last_synced_at || null, stale: status.status === "stale", sync_error: status.error || null, history_limited: Boolean(status.history_limited) };
}
...
module.exports = { MAX_PAGES, PAGE_SIZE, SYNC_SETTING, TIMEZONE, activityRow, activitySummary, isReply, isRepost, localDay, syncAccountActivity };
