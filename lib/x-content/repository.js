...
const WORKSPACE_TABLES = new Set([
  ...
  "x_daily_execution_plans", "x_daily_execution_plan_items", "x_scheduler_runs", "x_scheduler_leases"
]);
function config() {
...
async function getAutonomySchedule(id) { return first(await request(`x_autonomy_schedules?id=eq.${encodeURIComponent(id)}&select=*&limit=1`)); }
async function updateAutonomySchedule(id, changes) { return first(await request(`x_autonomy_schedules?id=eq.${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ ...changes, updated_at: new Date().toISOString() }) })); }
...
async function createExecutionPlanItem(item) { executionPlan.assertPlanItem(item); return first(await request("x_daily_execution_plan_items?on_conflict=workspace_id,plan_id,slot_number", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(item) })); }
...
async function listDueExecutionPlanItems(now = Date.now(), limit = 20) { const cutoff = new Date(now).toISOString(); return request(`x_daily_execution_plan_items?lifecycle_status=eq.scheduled&schedule_id=not.is.null&publication_id=is.null&intended_at=lte.${encodeURIComponent(cutoff)}&select=*&order=intended_at.asc&limit=${Math.min(100, Math.max(1, Number(limit) || 20))}`); }
async function listPublishingExecutionPlanItems(limit = 20) { return request(`x_daily_execution_plan_items?lifecycle_status=eq.publishing&schedule_id=not.is.null&select=*&order=updated_at.asc&limit=${Math.min(100, Math.max(1, Number(limit) || 20))}`); }
...
async function getExecutionPlanItemForPublication(publicationId) { return first(await request(`x_daily_execution_plan_items?publication_id=eq.${encodeURIComponent(publicationId)}&select=*&limit=1`)); }
async function updateExecutionPlanItem(id, changes) { return first(await request(`x_daily_execution_plan_items?id=eq.${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ ...changes, updated_at: new Date().toISOString() }) })); }
...
async function listAutonomyAuditsForSchedule(scheduleId, limit = 50) { return request(`x_autonomy_audit_events?schedule_id=eq.${encodeURIComponent(scheduleId)}&select=*&order=created_at.asc&limit=${Math.min(100, Math.max(1, Number(limit) || 50))}`); }
async function listAutonomyAuditsForDraft(draftId, limit = 50) { return request(`x_autonomy_audit_events?draft_id=eq.${encodeURIComponent(draftId)}&select=*&order=created_at.desc&limit=${Math.min(100, Math.max(1, Number(limit) || 50))}`); }
...
async function getGateAudit(id) { return first(await request(`x_gate_audits?id=eq.${encodeURIComponent(id)}&select=*&limit=1`)); }
...
async function claimSchedulerRun(trigger) {
  const context = tenant.requireCurrent();
  return first(await request("rpc/claim_x_scheduler_run", { method: "POST", body: JSON.stringify({
    p_workspace_id: context.workspaceId,
    p_scheduler_source: trigger.source,
    p_idempotency_key: trigger.idempotencyKey,
    p_intended_trigger_at: trigger.intendedTriggerAt,
    p_actual_trigger_at: trigger.actualTriggerAt,
    p_delay_ms: trigger.delayMs,
    p_lease_seconds: 240
  }) }));
}
async function finishSchedulerRun(id, status, result = {}) { return first(await request(`x_scheduler_runs?id=eq.${encodeURIComponent(id)}`, { method: "PATCH", body: JSON.stringify({ status, result, completed_at: new Date().toISOString(), updated_at: new Date().toISOString() }) })); }
async function releaseSchedulerLease(holderKey) { const context = tenant.requireCurrent(); return first(await request("rpc/release_x_scheduler_lease", { method: "POST", body: JSON.stringify({ p_workspace_id: context.workspaceId, p_lease_name: "autonomy_publish", p_holder_key: holderKey }) })); }
async function listSchedulerRuns(limit = 100) { return request(`x_scheduler_runs?select=*&order=actual_trigger_at.desc&limit=${Math.min(500, Math.max(1, Number(limit) || 100))}`); }
async function installPrimaryScheduler(cronSecret) { return first(await request("rpc/install_x_publisher_scheduler", { method: "POST", body: JSON.stringify({ p_cron_secret: cronSecret }) })); }
...
module.exports.getGateAudit = getGateAudit;
...
module.exports.listDueExecutionPlanItems = listDueExecutionPlanItems;
module.exports.listPublishingExecutionPlanItems = listPublishingExecutionPlanItems;
...
module.exports.getExecutionPlanItemForPublication = getExecutionPlanItemForPublication;
module.exports.updateExecutionPlanItem = updateExecutionPlanItem;
module.exports.getAutonomySchedule = getAutonomySchedule;
module.exports.listAutonomyAuditsForSchedule = listAutonomyAuditsForSchedule;
module.exports.listAutonomyAuditsForDraft = listAutonomyAuditsForDraft;
module.exports.claimSchedulerRun = claimSchedulerRun;
module.exports.finishSchedulerRun = finishSchedulerRun;
module.exports.releaseSchedulerLease = releaseSchedulerLease;
module.exports.listSchedulerRuns = listSchedulerRuns;
module.exports.installPrimaryScheduler = installPrimaryScheduler;
