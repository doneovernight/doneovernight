...
const WORKSPACE_TABLES = new Set([
  ...
  "x_post_performance_memory", "x_learning_reports", "x_growth_strategy_snapshots", "x_growth_decisions",
  ...
  "x_growth_experiments", "x_growth_executive_reports", "x_account_activity", "x_autonomy_decisions",
...
function scopedPath(path, method, body, context) {
  ...
  if (!tenant.workspaceScopingEnabled()) return { path, body };
...
async function request(path, options = {}) {
  ...
  try {
    const response = await fetch(`${url}/rest/v1/${prepared.path}`, { ...requestOptions, signal: controller.signal, headers: supabaseServiceHeaders(key, { "Content-Type": "application/json", Prefer: "return=representation", ...(options.headers || {}) }) });
