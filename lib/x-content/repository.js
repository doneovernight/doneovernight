...
const requestDiagnostics = new AsyncLocalStorage();
...
function diagnosticOperation(path, options = {}) {
  const [table = "", query = ""] = String(path || "").split("?", 2);
  const params = new URLSearchParams(query);
  const body = (() => { try { return JSON.parse(options.body || "{}"); } catch { return {}; } })();
  const method = String(options.method || "GET").toUpperCase();
  const preference = String(options.headers?.Prefer || "");
  return { table, operation: method === "GET" ? "select" : method === "PATCH" ? "update" : preference.includes("resolution=") ? "upsert" : "insert", columns: Object.keys(body).sort(), filters: [...params.keys()].filter((key) => !["select", "order", "limit", "on_conflict"].includes(key)).sort(), conflict_target: params.get("on_conflict") || null };
}
function diagnosticError(body) {
  try { const parsed = JSON.parse(body || "{}"); return { postgrest_code: parsed.code || null, postgres_code: /^\d{5}$/.test(String(parsed.code || "")) ? parsed.code : null, message: parsed.message || null, details: parsed.details || null, hint: parsed.hint || null }; }
  catch { return { postgrest_code: null, postgres_code: null, message: String(body || "").replace(/[\r\n]/g, " ").slice(0, 300) || null, details: null, hint: null }; }
}
function withSupabaseDiagnostics(callback) { return requestDiagnostics.run({ sequence: 0 }, callback); }
async function request(path, options = {}) {
  const diagnostic = requestDiagnostics.getStore();
  const operation = diagnostic ? { operation_number: ++diagnostic.sequence, ...diagnosticOperation(path, options) } : null;
  ...
  try {
    ...
    if (diagnostic) console.log("[X_CONTENT] autonomy_metrics_supabase", { ...operation, http_status: response.status, ...(response.ok ? { postgrest_code: null, postgres_code: null, message: null, details: null, hint: null } : diagnosticError(body)) });
...
async function savePerformanceMemory(row) { return first(await request("x_post_performance_memory?on_conflict=x_post_id", { method: "POST", headers: { Prefer: "resolution=merge-duplicates,return=representation" }, body: JSON.stringify(row) })); }
...
module.exports.withSupabaseDiagnostics = withSupabaseDiagnostics;
