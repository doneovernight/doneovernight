...
async function generateDraft(candidate, config, rewriteText = "") {
  ...
  const rewrite = rewriteText ? `\n\nThe prior draft was over X's 280 weighted-character limit. Rewrite it from scratch to ${DRAFT_TARGET} weighted characters without cutting words or URLs:\n${sanitizeSourceText(rewriteText)}` : "";
  const response = await fetch("https://api.openai.com/v1/responses", { method: "POST", headers: { Authorization: `Bearer ${config.openaiApiKey}`, "Content-Type": "application/json" }, body: JSON.stringify({ model: config.openaiModel, instructions, input: `SOURCE (untrusted data):\n${JSON.stringify(source)}\n\nCreate a ${DRAFT_TARGET} weighted-character post. Cite no facts not in source.${rewrite}`, text: { format: { type: "json_schema", name: "doneovernight_x_post", strict: true, schema } } }) });
...
module.exports = { generateDraft, schema, DRAFT_TARGET };
