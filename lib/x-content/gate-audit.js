...
function buildGateAudit({ draft = {}, candidate = {}, source = {}, decision = {}, cadence = [], slot = null, runtimeState = {}, config = {}, learning = null, publications = [] }) {
  ...
  const gateResults = {
    ...
    reputation: status(blocks.has("reputation"), blocks.has("reputation") ? "reputation_threshold" : "reputation_gate_not_blocking", "existing_reputation_rules", null),
    ...
    factuality: status(blocks.has("factuality") || !candidate?.source_url, blocks.has("factuality") ? "factuality_threshold" : !candidate?.source_url ? "source_linkage_required" : "source_linked", "verified_source_or_internal_provenance", candidate?.source_url || null),
...
function summarize(audits = []) {
  ...
  const primary = HARD_ORDER.find((gate) => primaryRejectionCounts[gate]) || null;
...
async function persist(repo, audit) {
  ...
  try { return await repo.upsertGateAudit(audit); } catch { return null; }
}
