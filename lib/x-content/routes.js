...
async function dailyPlan(req, res) {
  ...
  try { return send(res, 200, { success: true, result: await service.dailyAutonomyPlan({ skipDiscovery: requestUrl.searchParams.get("skip_discovery") === "1" }) }); } catch (error) { console.error("[X_CONTENT] daily_plan_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Daily plan failed", code: error.code || "DAILY_PLAN_FAILED" }); }
}
...
async function engagement(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.runEnrichmentCycle("engagement", () => service.engagementCheck()) }); } catch (error) { console.error("[X_CONTENT] engagement_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Engagement collection failed", code: error.code || "ENGAGEMENT_FAILED" }); }
}
...
async function autonomyPublish(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { const requestUrl = new URL(req.url || "/", `https://${req.headers.host || "doneovernight.com"}`); const result = requestUrl.searchParams.get("diagnostic") === "1" ? await service.autonomyPublishDiagnostic() : await service.schedulerPublishingCheck({ trigger: scheduler.triggerFromRequest(req) }); return send(res, 200, { success: true, result }); } catch (error) { console.error("[X_CONTENT] autonomy_publish_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Autonomy publishing check failed", code: error.code || "AUTONOMY_PUBLISH_FAILED" }); }
}
async function schedulerInstall(req, res) {
  if (req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.installPrimaryScheduler() }); }
  catch (error) { console.error("[X_CONTENT] scheduler_install_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Primary scheduler installation failed", code: error.code || "SCHEDULER_INSTALL_FAILED" }); }
}
async function autonomyMetrics(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.runEnrichmentCycle("analytics_learning", () => service.autonomyMetricsCheck()) }); } catch (error) { console.error("[X_CONTENT] autonomy_metrics_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Autonomy metrics check failed", code: error.code || "AUTONOMY_METRICS_FAILED" }); }
}
async function radar(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.runEnrichmentCycle("radar", () => service.socialRadarCycle()) }); } catch (error) { console.error("[X_CONTENT] radar_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Social Radar failed", code: error.code || "RADAR_FAILED" }); }
}
async function growth(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.runEnrichmentCycle("growth_director", () => service.growthDirectorCycle()) }); } catch (error) { console.error("[X_CONTENT] growth_director_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Growth Director failed", code: error.code || "GROWTH_DIRECTOR_FAILED" }); }
}
async function dailyBrief(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.runEnrichmentCycle("daily_brief", () => service.growthDailyBrief({ deliver: true })) }); } catch (error) { console.error("[X_CONTENT] daily_brief_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Daily brief failed", code: error.code || "DAILY_BRIEF_FAILED" }); }
}
async function intelligence(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.runEnrichmentCycle("growth_intelligence", () => service.growthIntelligenceCycle({ deliver: true })) }); } catch (error) { console.error("[X_CONTENT] growth_intelligence_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Growth Intelligence failed", code: error.code || "GROWTH_INTELLIGENCE_FAILED" }); }
}
async function executiveReport(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.runEnrichmentCycle("executive_report", () => service.monthlyExecutiveReport({ deliver: true })) }); } catch (error) { console.error("[X_CONTENT] executive_report_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Executive report failed", code: error.code || "EXECUTIVE_REPORT_FAILED" }); }
}
...
function oauthCookie(name, value, callbackUrl) {
  let domain = "";
  try {
    const hostname = new URL(callbackUrl).hostname;
    if (hostname === "doneovernight.com" || hostname.endsWith(".doneovernight.com")) domain = "; Domain=.doneovernight.com";
  } catch {}
  return `${name}=${encodeURIComponent(value)}; Max-Age=600; Path=/api/x-content-oauth/callback${domain}; HttpOnly; Secure; SameSite=Lax`;
}

function oauthCallbackPage(res, { status, code, redirectUrl }) {
  const target = new URL(redirectUrl);
  target.searchParams.set(status === "connected" ? "x_account" : "x_account_error", code || status);
  const targetOrigin = target.origin;
  const message = JSON.stringify({ type: "x-account-oauth", status, code: code || null });
  const safeTarget = JSON.stringify(target.toString());
  const safeOrigin = JSON.stringify(targetOrigin);
  res.statusCode = 200;
  res.setHeader("Content-Type", "text/html; charset=utf-8");
  res.setHeader("Cache-Control", "no-store");
  res.end(`<!doctype html><meta name="robots" content="noindex"><title>X account connection</title><script>(function(){var message=${message},target=${safeTarget},origin=${safeOrigin},hasOpener=window.opener&&window.opener!==window;try{if(hasOpener)window.opener.postMessage(message,origin)}catch(e){}if(hasOpener){window.close();window.setTimeout(function(){window.location.replace(target)},250)}else{window.location.replace(target)}})()</script><p>Returning to DONEOVERNIGHT…</p>`);
}
...
async function oauthCallback(req, res) {
  ...
  const adminUrl = "https://admin.doneovernight.com/x-content";
  const callbackUrl = process.env.X_REDIRECT_URI || "https://doneovernight.com/api/x-content-oauth/callback";
  const clearOAuthCookies = ["x_oauth2_callback_nonce", "x_oauth2_admin_binding"].map((key) => oauthCookie(key, "", callbackUrl).replace("Max-Age=600", "Max-Age=0"));
  if (requestUrl.searchParams.get("error")) { res.setHeader("Set-Cookie", clearOAuthCookies); await repository.setSetting("x_oauth2_connection_error", JSON.stringify({ code: "X_OAUTH2_DENIED", message: "Authorization was denied before consent.", at: new Date().toISOString() })).catch(() => null); return oauthCallbackPage(res, { status: "error", code: "X_OAUTH2_DENIED", redirectUrl: adminUrl }); }
  const cookies = Object.fromEntries(String(req.headers.cookie || "").split(";").map((part) => part.trim().split("=")).filter(([key, value]) => key && value).map(([key, ...value]) => [key, decodeURIComponent(value.join("="))]));
  try { await require("./x-client").completeOAuth2Authorization({ code, state, callbackNonce: cookies.x_oauth2_callback_nonce }, { workspaceId: require("./tenant-context").current()?.workspaceId, adminBinding: cookies.x_oauth2_admin_binding }); await service.recoverAfterOAuthReconnect().catch(() => null); res.setHeader("Set-Cookie", clearOAuthCookies); return oauthCallbackPage(res, { status: "connected", code: "connected", redirectUrl: adminUrl }); }
  catch (error) { res.setHeader("Set-Cookie", clearOAuthCookies); await repository.setSetting("x_oauth2_connection_error", JSON.stringify({ code: error.code || null, message: String(error.message || "").replace(/[\r\n]+/g, " ").slice(0, 240), at: new Date().toISOString() })).catch(() => null); return oauthCallbackPage(res, { status: "error", code: error.code || "X_OAUTH2_CALLBACK_FAILED", redirectUrl: adminUrl }); }
}
...
async function commandCenterSnapshot() {
  const config = await service.operationalConfig(); const accountActivity = await service.accountActivitySync().catch(() => null); const [drafts, publications, analytics, interactions, replyDrafts, sources, sourceControls, runs, autonomyState, dailyPlan, heartbeat, radarItems, growthStrategies, growthDecisions, xAccount, selfHealingState] = await Promise.all([listReviewDrafts(), safeArray(repository.listPublications(200)), safeArray(repository.listAnalytics(500)), safeArray(repository.listInteractions(200)), safeArray(repository.listReplyDrafts(200)), safeArray(repository.listSources(200)), safeArray(repository.listSourceControls(200)), safeArray(repository.listAgentRuns(100)), service.autonomyStatus().catch(() => null), service.dailyPlanStatus().catch(() => null), service.heartbeat().catch(() => null), safeArray(repository.listRadarItems(100)), safeArray(repository.listGrowthStrategySnapshots(10)), safeArray(repository.listGrowthDecisions(100)), service.xAccountStatus().catch(() => null), service.selfHealingStatus().catch(() => null)]);
  const publishedPosts = await listPublishedPostCards(publications);
  const system = { autonomy_mode: config.autonomy.mode, autonomous_publish_enabled: config.autonomy.publishEnabled, publish_mode: config.mode, publish_locked: config.autonomy.mode !== "auto" || !config.autonomy.publishEnabled, x_identity_configured: Boolean(config.x.accessToken || config.x.clientId), supabase_configured: Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY), openai_configured: Boolean(config.openaiApiKey && config.openaiModel), scheduler: "Supabase pg_cron every 5 minutes; GitHub Actions watchdog every 15 minutes; discovery every 2 hours", scheduler_health: heartbeat?.schedulerHealth || null, deployment_commit: String(process.env.VERCEL_GIT_COMMIT_SHA || "").slice(0, 12) || null, token_refresh_configured: Boolean(config.x.clientId && config.x.clientSecret) };
  system.core_publishing_health = heartbeat?.corePublishingHealth || null;
  system.enrichment_health = heartbeat?.enrichmentHealth || null;
  return { drafts, publications, published_posts: publishedPosts, analytics, interactions, reply_drafts: replyDrafts, sources, source_controls: sourceControls, runs, autonomy: autonomyState, self_healing: selfHealingState, daily_plan: dailyPlan, heartbeat, account_activity: accountActivity || heartbeat?.accountActivity || null, x_account: xAccount, radar: radarItems, growth: { strategies: growthStrategies, decisions: growthDecisions }, system };
}
...
async function admin(req, res) {
  ...
  try {
    ...
    else if (action === "x_account_status") result = await service.xAccountStatus();
    else if (action === "x_account_verify") result = await service.verifyXAccount();
    else if (action === "x_oauth_start") { const adminBinding = crypto.createHash("sha256").update(String(input.admin_key || "")).digest("hex"); const callbackUrl = process.env.X_REDIRECT_URI || "https://doneovernight.com/api/x-content-oauth/callback"; const started = await require("./x-client").startOAuth2Authorization({ workspaceId: require("./tenant-context").current()?.workspaceId || null, adminBinding }); res.setHeader("Set-Cookie", [oauthCookie("x_oauth2_callback_nonce", started.callbackNonce, callbackUrl), oauthCookie("x_oauth2_admin_binding", adminBinding, callbackUrl)]); result = { authorization_url: started.authorizationUrl, scopes: started.scopes, callback_url: callbackUrl }; }
    else if (action === "x_account_disconnect") { if (input.confirmation !== "DISCONNECT") return send(res, 400, { success: false, error: "Type DISCONNECT to remove the X connection" }); result = await service.disconnectXAccount(); }
...
const routeHandlers = { discover, dailyPlan, publish, heartbeat, engagement, regenerateLegacy, autonomyDecision, autonomyPublish, schedulerInstall, autonomyMetrics, radar, growth, dailyBrief, intelligence, executiveReport, identity, oauthStart, oauthCallback, telegram, admin };
