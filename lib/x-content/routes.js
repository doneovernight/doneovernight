...
async function dailyPlan(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.dailyAutonomyPlan() }); } catch (error) { console.error("[X_CONTENT] daily_plan_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Daily plan failed", code: error.code || "DAILY_PLAN_FAILED" }); }
}
