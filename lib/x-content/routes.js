...
async function identity(req, res) {
  ...
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try {
    const client = require("./x-client"); const application = await client.verifyIdentity().then((result) => ({ httpStatus: 200, username: result.username, userId: result.userId, authenticationMethod: result.authenticationMethod })).catch((error) => ({ httpStatus: error.statusCode || 500, code: error.code || "X_IDENTITY_FAILED", category: error.category || null, sanitizedResponse: client.sanitizeResponse(error.response) })); const isolated = await client.isolatedIdentityCheck();
    return send(res, application.httpStatus, { success: application.httpStatus === 200, application, isolated });
  } catch (error) {
...
module.exports = { discover, publish, heartbeat, identity, admin };
