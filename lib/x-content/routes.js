...
function sanitizedDatabaseDiagnostic(error) {
  try {
    ...
    return {
      code: String(detail?.code || "").replace(/[^A-Z0-9]/gi, "").slice(0, 16) || null,
...
async function autonomyMetrics(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.autonomyMetricsCheck() }); } catch (error) { const database = sanitizedDatabaseDiagnostic(error); console.error("[X_CONTENT] autonomy_metrics_failed", { message: error.message, code: error.code, database }); return send(res, error.statusCode || 500, { success: false, error: "Autonomy metrics check failed", code: error.code || "AUTONOMY_METRICS_FAILED", database }); }
}
...
module.exports = { discover, publish, heartbeat, engagement, regenerateLegacy, autonomyDecision, autonomyPublish, autonomyMetrics, radar, growth, dailyBrief, intelligence, executiveReport, identity, oauthStart, oauthCallback, telegram, admin };
