...
async function commandCenterSnapshot() {
  const config = await service.operationalConfig(); const [drafts, publications, analytics, interactions, replyDrafts, sources, sourceControls, runs, autonomyState, heartbeat, radarItems] = await Promise.all([listReviewDrafts(), safeArray(repository.listPublications(200)), safeArray(repository.listAnalytics(500)), safeArray(repository.listInteractions(200)), safeArray(repository.listReplyDrafts(200)), safeArray(repository.listSources(200)), safeArray(repository.listSourceControls(200)), safeArray(repository.listAgentRuns(100)), service.autonomyStatus().catch(() => null), service.heartbeat().catch(() => null), safeArray(repository.listRadarItems(100))]);
  const system = { autonomy_mode: config.autonomy.mode, autonomous_publish_enabled: config.autonomy.publishEnabled, publish_mode: config.mode, publish_locked: config.autonomy.mode !== "auto" || !config.autonomy.publishEnabled, x_identity_configured: Boolean(config.x.accessToken || config.x.clientId), supabase_configured: Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY), openai_configured: Boolean(config.openaiApiKey && config.openaiModel), scheduler: "Discovery every 2 hours; guarded autonomy check every 15 minutes", deployment_commit: String(process.env.VERCEL_GIT_COMMIT_SHA || "").slice(0, 12) || null, token_refresh_configured: Boolean(config.x.clientId && config.x.clientSecret) };
  return { drafts, publications, analytics, interactions, reply_drafts: replyDrafts, sources, source_controls: sourceControls, runs, autonomy: autonomyState, heartbeat, radar: radarItems, system };
}
...
module.exports = { discover, publish, heartbeat, engagement, regenerateLegacy, autonomyDecision, autonomyPublish, autonomyMetrics, radar, identity, oauthStart, oauthCallback, telegram, admin };
