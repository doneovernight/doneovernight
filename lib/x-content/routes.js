...
async function heartbeat(req, res) {
  ...
  try { return send(res, 200, { success: true, ...(await service.heartbeat()) }); } catch (error) { console.error("[X_CONTENT] heartbeat_failed", { message: error.message, statusCode: error.statusCode || null, detail: String(error.detail || "").slice(0, 300), workspaceScoping: require("./tenant-context").workspaceScopingEnabled() }); return send(res, error.statusCode || 500, { success: false, error: "Content heartbeat unavailable", code: error.code || "HEARTBEAT_FAILED" }); }
}
