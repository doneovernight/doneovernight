...
async function oauthCallback(req, res) {
  ...
  if (requestUrl.searchParams.get("error")) { res.statusCode = 303; res.setHeader("Location", "https://admin.doneovernight.com/x-content?x_account_error=X_OAUTH2_DENIED"); res.setHeader("Cache-Control", "no-store"); return res.end(); }
  ...
  const clearOAuthCookies = ["x_oauth2_callback_nonce", "x_oauth2_admin_binding"].map((key) => `${key}=; Max-Age=0; Path=/api/x-content-oauth/callback; HttpOnly; Secure; SameSite=Lax`);
  try { const result = await require("./x-client").completeOAuth2Authorization({ code, state, callbackNonce: cookies.x_oauth2_callback_nonce }, { workspaceId: require("./tenant-context").current()?.workspaceId, adminBinding: cookies.x_oauth2_admin_binding }); await service.recoverAfterOAuthReconnect().catch(() => null); res.setHeader("Set-Cookie", clearOAuthCookies); res.statusCode = 303; res.setHeader("Location", "https://admin.doneovernight.com/x-content?x_account=connected"); res.setHeader("Cache-Control", "no-store"); return res.end(); }
  catch (error) { res.setHeader("Set-Cookie", clearOAuthCookies); await repository.setSetting("x_oauth2_connection_error", JSON.stringify({ code: error.code || null, message: String(error.message || "").replace(/[\r\n]+/g, " ").slice(0, 240), at: new Date().toISOString() })).catch(() => null); res.statusCode = 303; res.setHeader("Location", `https://admin.doneovernight.com/x-content?x_account_error=${encodeURIComponent(error.code || "X_OAUTH2_CALLBACK_FAILED")}`); res.setHeader("Cache-Control", "no-store"); return res.end(); }
}
...
async function admin(req, res) {
  ...
  try {
    ...
    else if (action === "x_oauth_start") { const adminBinding = crypto.createHash("sha256").update(String(input.admin_key || "")).digest("hex"); const started = await require("./x-client").startOAuth2Authorization({ workspaceId: require("./tenant-context").current()?.workspaceId || null, adminBinding }); res.setHeader("Set-Cookie", [`x_oauth2_callback_nonce=${encodeURIComponent(started.callbackNonce)}; Max-Age=600; Path=/api/x-content-oauth/callback; HttpOnly; Secure; SameSite=Lax`, `x_oauth2_admin_binding=${encodeURIComponent(adminBinding)}; Max-Age=600; Path=/api/x-content-oauth/callback; HttpOnly; Secure; SameSite=Lax`]); result = { authorization_url: started.authorizationUrl, scopes: started.scopes, callback_url: process.env.X_REDIRECT_URI || "https://doneovernight.com/api/x-content-oauth/callback" }; }
