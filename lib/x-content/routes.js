...
async function oauthCallback(req, res) {
  ...
  try { const result = await require("./x-client").completeOAuth2Authorization({ code, state, callbackNonce: cookies.x_oauth2_callback_nonce }, { workspaceId: require("./tenant-context").current()?.workspaceId, adminBinding: cookies.x_oauth2_admin_binding }); const recovery = await service.recoverAfterOAuthReconnect().catch((error) => ({ error: error.code || "X_OAUTH2_RECOVERY_FAILED" })); res.setHeader("Set-Cookie", clearOAuthCookies); return send(res, 200, { success: true, username: result.username, user_id: result.userId, scopes: result.scopes, refresh_token_available: result.refreshTokenAvailable, refreshed: result.refreshed, recovery }); }
  catch (error) { res.setHeader("Set-Cookie", clearOAuthCookies); await repository.setSetting("x_oauth2_connection_error", JSON.stringify({ code: error.code || null, message: String(error.message || "").replace(/[\r\n]+/g, " ").slice(0, 240), at: new Date().toISOString() })).catch(() => null); return send(res, error.statusCode || 500, { success: false, error: "X OAuth 2.0 authorization could not be completed", code: error.code || "X_OAUTH2_CALLBACK_FAILED" }); }
}
