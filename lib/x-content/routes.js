...
async function oauthStatus(req, res) {
  if (req.method !== "GET") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  return send(res, 200, { success: true, ...require("./x-client").oauth2RuntimeStatus() });
}
...
module.exports = { discover, publish, heartbeat, identity, oauthStart, oauthStatus, oauthCallback, admin };
