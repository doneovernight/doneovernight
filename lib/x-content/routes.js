...
async function regenerateLegacy(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  try { return send(res, 200, { success: true, result: await service.regenerateAllLegacyDrafts() }); } catch (error) { console.error("[X_CONTENT] legacy_regeneration_failed", { message: error.message, code: error.code }); return send(res, error.statusCode || 500, { success: false, error: "Legacy regeneration failed", code: error.code || "LEGACY_REGENERATION_FAILED" }); }
}
...
async function listReviewDrafts() {
  ...
  return Promise.all(drafts.map(async (draft) => {
    ...
    return { id: draft.id, text: draft.text, weighted_character_count: draft.weighted_character_count, post_type: v2.format_label || draft.post_type, topic: draft.topic_cluster || candidate?.topic_cluster || "", source_title: source?.title || candidate?.headline || "Official source", source_url: sourceUrl(draft, candidate), confidence: draft.confidence, quality_score: draft.quality_score, insight_score: v2.scores?.insight ?? null, save_score: v2.scores?.save ?? null, repost_score: v2.scores?.repost ?? null, educational_score: v2.scores?.educational ?? null, brand_alignment: v2.scores?.brand ?? null, mention_preview: v2.mention_preview || null, predicted_performance: v2.scores ? v2.scores.quality : draft.quality_score, reply_suggestions: replies.map((reply) => ({ id: reply.id, text: reply.text, classification: reply.classification, confidence: reply.confidence, status: reply.status })), created_at: draft.created_at, status: draft.status };
  }));
}
...
module.exports = { discover, publish, heartbeat, engagement, regenerateLegacy, identity, oauthStart, oauthCallback, admin };
