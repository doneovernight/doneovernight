...
function autonomyDiagnosticError(error) {
  let body = null;
  try { body = JSON.parse(String(error?.detail || "")); } catch { body = null; }
  const stack = String(error?.stack || "");
  const location = stack.match(/(\/lib\/x-content\/[^:\n]+:\d+:\d+)/)?.[1] || null;
  return {
    error_class: error?.constructor?.name || "Error",
    message: String(error?.message || "").slice(0, 240),
    status: Number(error?.statusCode) || null,
    postgrest_code: body?.code || null,
    postgres_code: body?.code || null,
    details: typeof body?.details === "string" ? body.details.slice(0, 240) : null,
    hint: typeof body?.hint === "string" ? body.hint.slice(0, 240) : null,
    stack_location: location
  };
}

const AUTONOMY_REPO_OPS = {
  getSetting: ["A", "select", "x_settings"],
  createRun: ["B", "insert", "x_agent_runs"],
  finishRun: ["L", "update", "x_agent_runs"],
  recordAutonomyAudit: ["C", "insert", "x_autonomy_audit_events"],
  listAutonomySchedules: ["D", "select", "x_autonomy_schedules"],
  getDraft: ["E", "select", "x_drafts"],
  getCandidate: ["F", "select", "x_topic_candidates"],
  updateDraft: ["G", "update", "x_drafts"],
  updateAutonomySchedule: ["I", "update", "x_autonomy_schedules"],
  createPublication: ["H", "insert", "x_publications"],
  updatePublication: ["J", "update", "x_publications"],
  setSetting: ["K", "upsert", "x_settings"]
};

function autonomyDiagnosticRepository(base, diagnostics) {
  return new Proxy(base, { get(target, property, receiver) {
    const value = Reflect.get(target, property, receiver);
    if (typeof value !== "function" || !AUTONOMY_REPO_OPS[property]) return value;
    return async (...args) => {
      const [phase, operation, table] = AUTONOMY_REPO_OPS[property];
      const fields = property === "createRun" ? ["run_type", "status", "started_at"]
        : property === "finishRun" ? ["status", "completed_at", "summary", "error_message"]
          : property === "getSetting" ? ["key"]
            : property === "listAutonomySchedules" ? ["*"]
              : Array.isArray(args[1]) ? args[1].map(String).slice(0, 40) : (args[1] && typeof args[1] === "object" ? Object.keys(args[1]).slice(0, 40) : []);
      const record = { operation_number: diagnostics.records.length + 1, phase, operation, table, columns: fields, filters: property === "getSetting" ? ["key"] : property === "getDraft" || property === "getCandidate" || property === "finishRun" || property === "updateDraft" || property === "updatePublication" || property === "updateAutonomySchedule" ? ["id"] : [] };
      diagnostics.records.push(record); diagnostics.x_agent_runs_attempted ||= table === "x_agent_runs" && operation !== "select";
      try { const result = await value.apply(target, args); record.http_status = 200; record.result_present = result !== null && result !== undefined; return result; }
      catch (error) { Object.assign(record, autonomyDiagnosticError(error)); throw error; }
    };
  } });
}
...
async function autonomyPublish(req, res) {
  if (req.method !== "GET" && req.method !== "POST") return send(res, 405, { success: false, error: "Method not allowed" });
  if (!isCron(req)) return send(res, 401, { success: false, error: "Cron authorization required" });
  const diagnostics = { request_id: randomUUID(), workspace_id: null, x_account_id: null, x_agent_runs_attempted: false, records: [] };
  try {
    const tenantContext = require("./tenant-context").current();
    diagnostics.workspace_id = tenantContext?.workspaceId || null;
    diagnostics.x_account_id = tenantContext?.xAccountId || null;
    const diagnosticRepository = autonomyDiagnosticRepository(repository, diagnostics);
    return send(res, 200, { success: true, result: await service.autonomyPublishingCheck({ repository: diagnosticRepository }) });
  } catch (error) {
    const failure = autonomyDiagnosticError(error);
    console.error("[X_CONTENT] autonomy_publish_diagnostic", { request_id: diagnostics.request_id, workspace_id: diagnostics.workspace_id, x_account_id: diagnostics.x_account_id, x_agent_runs_attempted: diagnostics.x_agent_runs_attempted, records: diagnostics.records, failure });
    return send(res, error.statusCode || 500, { success: false, error: "Autonomy publishing check failed", code: error.code || "AUTONOMY_PUBLISH_FAILED", diagnostic: { request_id: diagnostics.request_id, workspace_id: diagnostics.workspace_id, x_account_id: diagnostics.x_account_id, x_agent_runs_attempted: diagnostics.x_agent_runs_attempted, records: diagnostics.records, failure } });
  }
}
...
const wrappedHandlers = Object.fromEntries(Object.entries(routeHandlers).map(([name, handler]) => [name, async (req, res) => {
  try {
    ...
    if (name === "autonomyPublish") {
      const diagnostic = { request_id: randomUUID(), x_agent_runs_attempted: false, records: [], failure: autonomyDiagnosticError(error) };
      console.error("[X_CONTENT] autonomy_publish_boundary_diagnostic", diagnostic);
      return send(res, status, { success: false, error: status === 403 ? "Workspace context required" : "X content route unavailable", code: error.code || "X_CONTENT_ROUTE_FAILED", diagnostic });
    }
