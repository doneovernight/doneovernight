...
const EXPECTED_OAUTH2_REDIRECT_URI = "https://doneovernight.com/api/x-content-oauth/callback";
...
function oauth2RuntimeStatus() {
  const status = (name) => {
    ...
    return {
      ...
      plausible_format: present && value.trim() === value && /^[\x21-\x7E]+$/.test(value)
    };
  };
  const config = getConfig().x;
  const clientId = process.env.X_CLIENT_ID || "";
  const authorizationUrl = clientId ? buildOAuth2AuthorizationUrl({ clientId, redirectUri: config.redirectUri, state: "diagnostic", verifier: "diagnostic" }) : null;
  return {
    client_id: status("X_CLIENT_ID"),
    client_secret: status("X_CLIENT_SECRET"),
    authorization_url_uses_runtime_client_id: Boolean(authorizationUrl && new URL(authorizationUrl).searchParams.get("client_id") === clientId),
    redirect_uri_matches_expected: config.redirectUri === EXPECTED_OAUTH2_REDIRECT_URI,
    requested_scopes: OAUTH2_SCOPES,
    requested_scopes_valid: OAUTH2_SCOPES.join(" ") === "tweet.read tweet.write users.read offline.access"
  };
}
...
module.exports = { OAUTH2_SCOPES, authenticationMethod, buildOAuth2AuthorizationUrl, completeOAuth2Authorization, oauth1Header, oauth2RuntimeStatus, publish, refreshAccessToken, startOAuth2Authorization, verifyIdentity, verifyOAuth2User };
