...
async function storedOAuth2Connection(options = {}) {
  ...
  if (!options.forceRefresh && Date.now() + REFRESH_EARLY_MS < new Date(connection.expires_at).getTime()) return connection;
...
async function verifyIdentity(options) { const result = await request("GET", "/2/users/me?user.fields=username,name", null, { ...(options || {}), failurePhase: "identity" }); const username = String(result.data?.data?.username || "").toLowerCase(); const userId = String(result.data?.data?.id || ""); if (username !== "doneovernight" || userId !== REQUIRED_X_USER_ID) { const error = new Error(`X identity guard failed: expected @doneovernight (${REQUIRED_X_USER_ID}), got @${username || "unknown"} (${userId || "unknown"})`); error.code = "X_USERNAME_GUARD_FAILED"; throw error; } await repository.setSetting(OAUTH2_LAST_IDENTITY_KEY, JSON.stringify({ at: new Date().toISOString(), username, user_id: userId })).catch(() => null); return { username, userId, authenticationMethod: result.authenticationMethod || authenticationMethod(options), refreshed: result.refreshed, refreshTokenRotated: result.refreshTokenRotated }; }
