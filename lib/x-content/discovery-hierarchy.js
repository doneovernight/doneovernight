...
function scoreDiscoveryCandidate(candidate, context = {}) {
  ...
  const existing = [...(context.existingCandidates || []), ...(context.recentDrafts || [])].filter((row) => {
    if (row === candidate) return false;
    if (candidate.id != null && row?.id != null && String(row.id) === String(candidate.id)) return false;
    return true;
  });
...
function internalKnowledgeCandidate(knowledge, now = Date.now()) {
  ...
  const id = knowledge.id || `internal-${now}`; const sourceUrl = knowledge.source_url || `https://doneovernight.com/internal-knowledge/${encodeURIComponent(id)}`;
  return { id, title: knowledge.title || "DONEOVERNIGHT operating insight", summary: knowledge.evidence || knowledge.reason || "Workspace-provided operating knowledge.", source_url: sourceUrl, sourceUrl, publisher: "DONEOVERNIGHT", topic_cluster: knowledge.topic_cluster || knowledge.topic || "internal knowledge", publishedAt: knowledge.updated_at || knowledge.created_at || new Date(now).toISOString(), trust_score: 0.92, authority_score: 0.9, relevance_score: Number(knowledge.relevance_score || 0.82), quality_score: Number(knowledge.quality_score || 0.84), novelty_score: Number(knowledge.novelty_score || 0.8), internal_provenance: { kind: "workspace_knowledge", evidence: knowledge.evidence || knowledge.reason || "Provided by the active workspace", selected_at: new Date(now).toISOString() } };
}
