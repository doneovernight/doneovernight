...
async function dailyAutonomyPlan(options = {}) {
  ...
  try {
    ...
    const linkedDraftIds = new Set(allExistingPlanItems.map((item) => item.draft_id).filter(Boolean).map(String)); const detachedDrafts = drafts.filter((draft) => draft.status === "queued" && !isLegacyDraft(draft) && !linkedDraftIds.has(String(draft.id)) && !publicationDrafts.has(draft.id) && Number.isFinite(new Date(draft.created_at).getTime()) && now - new Date(draft.created_at).getTime() <= FRESHNESS_MS).sort((left, right) => Number(right.quality_score || 0) - Number(left.quality_score || 0));
    ...
    for (const slot of plan.slots) {
      ...
      for (const unselected of viable.slice(1)) await repo.updateDraft(unselected.id, { status: "rejected", rejection_reason: "Superseded by the highest-quality draft generated for the same canonical slot" });
...
async function heartbeat() {
  ...
  const [discovery, publishing, autonomyPublishing, failedAutonomyPublishing, autonomyDecision, drafts, publications, activityRows, activitySetting, safeStopSetting, schedules] = await Promise.all([
    repository.latestRun("discovery"), repository.latestRun("publishing"), repository.latestRun("autonomy_publish"), (repository.latestFailedRun ? repository.latestFailedRun("autonomy_publish") : Promise.resolve(null)).catch(() => null), repository.latestRun("autonomy"), repository.recentDrafts(), repository.publicationsToday(),
  ...
  const now = Date.now();
  const [canonicalStatus, tokenMetadata, radarRun, engagementRun, metricsRun, growthRun] = await Promise.all([dailyPlanStatus().catch(() => null), xClient.storedOAuth2Metadata().catch(() => null), repository.latestRun("radar").catch(() => null), repository.latestRun("engagement").catch(() => null), repository.latestRun("autonomy_metrics").catch(() => null), repository.latestRun("growth_director").catch(() => null)]);
...
async function autonomyPublishingCheck(options = {}) {
  ...
  try { const summary = await autonomy.processScheduled({ ...options, repository: repo, config, runId: run.id, notify: options.notify || notify, requireCanonicalPlan: canonicalExecutionPlanEnabled() }); await repo.finishRun(run.id, "completed", summary); if (summary.published) summary.enrichment = await refreshPostPublishEnrichment(repo); return summary; }
...
module.exports = { discover, dailyAutonomyPlan, dailyPlanStatus, publishNext, publishApprovedDraft, approveDraft, rejectDraft, regenerateDraft, regenerateAllLegacyDrafts, markLegacyDrafts, deleteDraft, editDraft, scheduleDraft, testPost, heartbeat, engagementCheck, accountActivitySync, operationalConfig, canPublish, jaccard, backfillDrafts, backfillSkipReason, generateAndStoreDraft, runEnrichmentCycle, autonomyDecisionCycle, autonomyPublishingCheck, autonomyPublishDiagnostic, recoverAfterOAuthReconnect, scheduledPublishingCheck, autonomyMetricsCheck, autonomyStatus, selfHealingStatus, xAccountStatus, verifyXAccount, disconnectXAccount, learningShadowCycle, socialRadarCycle, growthDirectorCycle, growthDailyBrief, growthIntelligenceCycle, monthlyExecutiveReport, refreshEditorProfile, recordEditorFeedback, discoveryHierarchy: HIERARCHY, scoreDiscoveryCandidate, selectHierarchicalCandidate, autonomy };
