...
function v2CandidateReason(candidate, candidateDrafts, publicationsByDraft, activeDrafts, config, now) {
  ...
  if (activeDrafts.some((draft) => draft.topic_cluster === candidate.topic_cluster && draft.status !== "rejected" && now - new Date(draft.created_at).getTime() <= TOPIC_COOLDOWN_MS)) return "topic_cooldown";
...
async function regenerateAllLegacyDrafts(options = {}) {
  ...
  const attempt = async (candidate, legacyDraft = null) => {
    if (!candidate || selected.has(candidate.id) || summary.generated.length >= limit) return;
    const reason = v2CandidateReason(candidate, byCandidate.get(candidate.id), publicationsByDraft, activeDrafts, config, now); if (reason) { summary.skipped[reason] = (summary.skipped[reason] || 0) + 1; return; }
...
module.exports = { discover, publishNext, publishApprovedDraft, approveDraft, rejectDraft, regenerateDraft, regenerateAllLegacyDrafts, markLegacyDrafts, testPost, heartbeat, engagementCheck, operationalConfig, canPublish, jaccard, backfillDrafts, backfillSkipReason, generateAndStoreDraft };
