...
async function dailyAutonomyPlan(options = {}) {
  ...
  try {
    const planDate = dailyPlan.dayKey(new Date(now), config.timezone); const marker = repo.getSetting ? await repo.getSetting("x_daily_plan_date").catch(() => null) : null;
    ...
    for (const schedule of existingSchedules) {
      ...
      if (schedule.status === "scheduled" && Number.isFinite(at) && at < now && !publicationDrafts.has(schedule.draft_id)) { await repo.updateAutonomySchedule(schedule.id, { status: "cancelled", reason: "stale_daily_plan_replaced" }).catch(() => null); result.stale_schedules_cancelled.push(schedule.id); await autonomy.audit(repo, { event_type: "schedule_cancelled", schedule_id: schedule.id, draft_id: schedule.draft_id, mode: config.autonomy.mode, reason: "stale_daily_plan_replaced", created_at: new Date(now).toISOString() }); }
    }
