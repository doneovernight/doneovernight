...
async function currentExecutionPlan(repo, now, config) {
  if (!canonicalExecutionPlanEnabled() || typeof repo.createExecutionPlan !== "function") return null;
  const planDate = executionPlan.dateKey(new Date(now), config.timezone);
  try {
    return await repo.createExecutionPlan({ plan_date: planDate, timezone: config.timezone, minimum_posts: config.autonomy.minimumDailyTarget || 2, preferred_posts: config.autonomy.preferredDailyRange?.[0] || 3, maximum_posts: config.autonomy.dailyCap || 5, status: "open" });
  } catch { return null; }
}
async function readExecutionPlan(repo, now, config) {
  ...
  try { return await repo.getExecutionPlan(executionPlan.dateKey(new Date(now), config.timezone)); } catch { return null; }
}
async function ensureExecutionPlanItem(repo, plan, fields) {
  if (!plan || typeof repo.createExecutionPlanItem !== "function") return null;
  try { return await repo.createExecutionPlanItem({ plan_id: plan.id, slot_number: fields.slot_number, intended_at: fields.intended_at || null, candidate_id: fields.candidate_id || null, draft_id: fields.draft_id || null, lifecycle_status: fields.lifecycle_status || "drafted" }); } catch { return null; }
}
...
function candidateFromRow(row = {}) {
  return { ...row, title: row.title || row.headline, summary: row.summary || row.evidence_summary || "Verified workspace operating context.", sourceUrl: row.sourceUrl || row.source_url, source_url: row.source_url || row.sourceUrl, publisher: row.publisher || "DONEOVERNIGHT", publishedAt: row.publishedAt || row.published_at || row.created_at, trust_score: row.trust_score || row.trustScore || .9, authority_score: row.authority_score || row.authority || .9, relevance_score: row.relevance_score || row.relevance || .82, quality_score: row.quality_score || row.quality || .84, novelty_score: row.novelty_score || row.novelty || .8, discovery_tier: row.discovery_tier || "internal_knowledge" };
}

async function dailyAutonomyPlan(options = {}) {
  const repo = options.repository || repository; const config = options.config || await operationalConfig(); const now = options.now || Date.now(); const run = await repo.createRun("autonomy"); const result = { plan: null, discovery: null, stale_schedules_cancelled: [], slots: [], drafts_created: 0, drafts_selected: 0, schedules_created: 0, auto_approved: 0, at_risk: false, blocker: null, skipped: null };
  try {
    const planDate = dailyPlan.dayKey(new Date(now), config.timezone); const canonicalPlan = await currentExecutionPlan(repo, now, config); const marker = repo.getSetting ? await repo.getSetting("x_daily_plan_date_v2").catch(() => null) : null;
    if (marker?.value === planDate && canonicalPlan && repo.listExecutionPlanItems) {
      result.skipped = "Canonical daily execution plan already generated for this Amsterdam day"; result.plan = dailyPlan.planSlots({ now, timezone: config.timezone, count: dailyPlan.PREFERRED_RANGE[0] }); const items = await repo.listExecutionPlanItems(canonicalPlan.id).catch(() => []); result.slots = items.map((item) => ({ plan_id: canonicalPlan.id, plan_item_id: item.id, slot_number: item.slot_number, intended_at: item.intended_at, draft_id: item.draft_id, candidate_id: item.candidate_id, schedule_id: item.schedule_id, status: item.lifecycle_status })); const existingPublications = await repo.listPublishedPublications(200).catch(() => []); const publishedToday = existingPublications.filter((row) => row.status === "published" && dailyPlan.dayKey(new Date(row.published_at), config.timezone) === planDate).length; const scheduledToday = items.filter((item) => ["scheduled", "publishing"].includes(item.lifecycle_status)).length; const next = items.filter((item) => item.lifecycle_status === "scheduled" && new Date(item.intended_at || 0).getTime() >= now).sort((left, right) => new Date(left.intended_at) - new Date(right.intended_at))[0]?.intended_at || null; result.status = dailyPlan.dailyStatus({ published: publishedToday, scheduled: scheduledToday, blocker: result.skipped, next }); result.at_risk = result.status.at_risk; result.blocker = result.status.blocker; await repo.finishRun(run.id, "completed", result); return result;
    }
    if (marker?.value === planDate) { result.skipped = "Daily plan already generated for this Amsterdam day"; result.plan = dailyPlan.planSlots({ now, timezone: config.timezone, count: dailyPlan.PREFERRED_RANGE[0] }); const existingPublications = await repo.listPublishedPublications(200).catch(() => []); const existingDaySchedules = await repo.listAutonomySchedules(200).catch(() => []); const publishedToday = existingPublications.filter((row) => row.status === "published" && dailyPlan.dayKey(new Date(row.published_at), config.timezone) === planDate).length; const scheduledToday = existingDaySchedules.filter((row) => ["scheduled", "shadow"].includes(row.status) && dailyPlan.dayKey(new Date(row.scheduled_for), config.timezone) === planDate).length; const next = existingDaySchedules.filter((row) => row.status === "scheduled" && new Date(row.scheduled_for).getTime() >= now).sort((left, right) => new Date(left.scheduled_for) - new Date(right.scheduled_for))[0]?.scheduled_for || null; result.status = dailyPlan.dailyStatus({ published: publishedToday, scheduled: scheduledToday, blocker: result.skipped, next }); result.at_risk = result.status.at_risk; result.blocker = result.status.blocker; await repo.finishRun(run.id, "completed", result); return result; }
    result.discovery = await discover().catch((error) => ({ error: error.code || "DISCOVERY_FAILED" }));
    const existingSchedules = await repo.listAutonomySchedules(200).catch(() => []); const publications = await repo.listPublishedPublications(200).catch(() => []); const publicationDrafts = new Set(publications.map((row) => row.draft_id));
    for (const schedule of existingSchedules) {
      const at = new Date(schedule.scheduled_for || 0).getTime();
      const scheduleDate = Number.isFinite(at) ? dailyPlan.dayKey(new Date(at), config.timezone) : null; const replaceSameDay = scheduleDate === planDate && ["daily_autonomous_plan", "promoted_from_shadow", "recovered_after_oauth_reconnect"].includes(schedule.reason);
      if (schedule.status === "scheduled" && Number.isFinite(at) && !publicationDrafts.has(schedule.draft_id) && (at < now || replaceSameDay)) { await repo.updateAutonomySchedule(schedule.id, { status: "cancelled", reason: "stale_daily_plan_replaced" }).catch(() => null); result.stale_schedules_cancelled.push(schedule.id); await autonomy.audit(repo, { event_type: "schedule_cancelled", schedule_id: schedule.id, draft_id: schedule.draft_id, mode: config.autonomy.mode, reason: "stale_daily_plan_replaced", created_at: new Date(now).toISOString() }); }
    }
    const plan = dailyPlan.planSlots({ now, timezone: config.timezone, count: dailyPlan.PREFERRED_RANGE[0] }); result.plan = plan;
    const candidates = (await repo.recentCandidates().catch(() => [])).map(candidateFromRow).filter((row) => row.status === "accepted" && new Date(row.publishedAt).getTime() >= now - 7 * 86_400_000); const fallbackRows = await hierarchicalFallbackRows(repo); const pools = [...candidates, ...fallbackRows]; const usedTopics = new Set(); const drafts = await repo.listDrafts(200).catch(() => []);
    const generationConfig = { ...config, v2DraftBatchSize: 1 };
    for (const slot of plan.slots) {
      const eligible = pools.filter((candidate) => !usedTopics.has(String(candidate.topic_cluster || candidate.topic || "").toLowerCase()) && slot.discovery_tiers.includes(candidate.discovery_tier)).filter((candidate) => scoreDiscoveryCandidate(candidate, { existingCandidates: candidates, recentDrafts: drafts, recentPublications: publications, now }).eligible); const candidate = eligible[0] || pools.filter((row) => !usedTopics.has(String(row.topic_cluster || row.topic || "").toLowerCase()) && row.source_url).find((row) => scoreDiscoveryCandidate(row, { existingCandidates: candidates, recentDrafts: drafts, recentPublications: publications, now }).eligible) || null;
      if (!candidate) { result.slots.push({ ...slot, status: "blocked", reason: "No candidate passed hierarchy and fallback gates", fallback_tier: slot.fallback_tier }); continue; }
      ...
      if (!selected) { result.slots.push({ ...slot, status: "blocked", topic: candidate.topic_cluster, reason: "All three candidates failed quality or safety gates", fallback_tier: slot.fallback_tier }); continue; }
      result.drafts_selected += 1;
      if (canonicalPlan) {
        const item = await ensureExecutionPlanItem(repo, canonicalPlan, { slot_number: slot.index, intended_at: slot.planned_for, candidate_id: workingCandidate.id || candidate.id, draft_id: selected.id, lifecycle_status: "drafted" });
        result.slots.push({ ...slot, status: item ? "drafted" : "blocked", topic: candidate.topic_cluster, discovery_tier: candidate.discovery_tier, confidence: evaluation.confidence, draft_id: selected.id, plan_id: canonicalPlan.id, plan_item_id: item?.id || null, schedule_id: null, source_url: candidate.source_url, draft_text: selected.text, weighted_character_count: selected.weighted_character_count, why_selected: "Highest-scoring candidate from the highest available hierarchy tier that passed all gates." });
      } else {
    ...
    const publishedToday = publications.filter((row) => row.status === "published" && new Intl.DateTimeFormat("en-CA", { timeZone: config.timezone }).format(new Date(row.published_at)) === new Intl.DateTimeFormat("en-CA", { timeZone: config.timezone }).format(new Date(now))).length; const scheduledToday = result.slots.filter((slot) => ["scheduled", "shadow"].includes(slot.status)).length; const status = dailyPlan.dailyStatus({ published: publishedToday, scheduled: scheduledToday, blocker: "No fresh candidate or approved internal provenance passed all gates", next: result.slots.find((slot) => slot.status === "scheduled")?.planned_for || null }); result.status = status; result.at_risk = status.at_risk; result.blocker = status.blocker; if (repo.setSetting) await repo.setSetting("x_daily_plan_date_v2", planDate).catch(() => null); await repo.finishRun(run.id, "completed", result); return result;
...
async function scheduleDraft(id, scheduledFor, options = {}) {
  ...
  const config = await operationalConfig(); const plan = await readExecutionPlan(repository, at.getTime(), config); const planItem = plan && repository.getExecutionPlanItemForDraft ? await repository.getExecutionPlanItemForDraft(id).catch(() => null) : null;
  if (plan && !planItem) { const error = new Error("A canonical execution-plan item is required before scheduling"); error.code = "EXECUTION_PLAN_REQUIRED"; throw error; }
...
async function heartbeat() {
  ...
  const [discovery, publishing, autonomyPublishing, failedAutonomyPublishing, drafts, publications, activityRows, activitySetting, safeStopSetting, schedules] = await Promise.all([
    ...
    repository.listAccountActivity(1000).catch(() => null), repository.getSetting(accountActivity.SYNC_SETTING).catch(() => null), repository.getSetting(autonomy.SAFE_STOP_KEY).catch(() => null), repository.listAutonomySchedules(200).catch(() => [])
  ]);
  ...
  const now = Date.now();
  const overdueSchedules = schedules.filter((row) => row.status === "scheduled" && Number.isFinite(new Date(row.scheduled_for || 0).getTime()) && new Date(row.scheduled_for).getTime() <= now);
  const overdueBeyondGrace = overdueSchedules.filter((row) => now - new Date(row.scheduled_for).getTime() > autonomy.scheduleGraceMs(config)).length;
  ...
  const schedulerLate = autonomousMode && (!latestRunAt || now - latestRunAt > 30 * 60000);
  const publishingHealthy = autonomousMode ? autonomyPublishing?.status === "completed" && !safeStop && !(latestFailureAt > latestSuccessfulAt) && overdueBeyondGrace === 0 && !schedulerLate : autonomyPublishing ? autonomyPublishing.status === "completed" : publishing ? publishing.status === "completed" : null;
  return { lastSuccessfulDiscovery: discovery?.status === "completed" ? discovery.completed_at : null, lastGeneratedDraft: drafts[0]?.created_at || null, lastSuccessfulPublication: publications.find((item) => item.status === "published")?.published_at || null, currentMode: config.mode, postsPublishedToday: activity.posts_today, agentPublishedToday: activity.agent_published_today, manualPostsToday: activity.manual_posts_today, accountActivity: activity, nextScheduledRun: "Discovery every 2 hours; publishing check every 15 minutes", publishingCheckStatus: publishingStatus, publishingHealthy, publishingHealth: { status: publishingHealthy === false ? "degraded" : "healthy", overdue_schedules: overdueSchedules.map((row) => ({ id: row.id, draft_id: row.draft_id, scheduled_for: row.scheduled_for, overdue_minutes: Math.max(0, Math.round((now - new Date(row.scheduled_for).getTime()) / 60000)) })), overdue_beyond_grace: overdueBeyondGrace, scheduler_late: schedulerLate, last_scheduler_run: latestRunAt ? new Date(latestRunAt).toISOString() : null, recovery_action: overdueBeyondGrace ? "run guarded publisher and transition due schedules" : null }, latestError: publishingFailure?.error_message || [discovery].find((item) => item?.status === "failed")?.error_message || null, lastFailedPublishingCheck: publishingFailure?.completed_at || publishingFailure?.started_at || null };
}
...
async function autonomyDecisionCycle(options = {}) {
  ...
  try { const summary = await autonomy.runAutonomyCycle({ ...options, repository: repo, config, runId: run.id, notify: options.notify || notify }); summary.learning = await learningShadowCycle({ repository: repo }); await repo.finishRun(run.id, "completed", summary); return summary; }
    ...
    // The gate-audit migration is additive and may be applied after the
    // application deploy. Treat that specific schema-cache miss as a safe,
    // successful no-op so the scheduled workflow does not turn an optional
    // shadow decision into a repeated HTTP 404. No schedules or publications
    // are created until the migration is available.
    const detail = String(error.detail || error.message || "");
    if (error.statusCode === 404 && /x_gate_audits/i.test(detail)) {
      const result = { schema_pending: true, skipped: "Gate-audit schema is unavailable; shadow decisions were not persisted", migration: "20260724_x_gate_audit.sql", evaluated: 0, scheduled: [], published: false };
      await autonomy.audit(repo, { event_type: "cycle_completed", run_id: run.id, mode: config.autonomy?.mode || config.mode, reason: "gate_audit_schema_pending" }).catch(() => null);
      await repo.finishRun(run.id, "completed", result).catch(() => null);
      return result;
    }
    await autonomy.audit(repo, { event_type: "cycle_completed", run_id: run.id, mode: config.autonomy.mode, reason: "autonomy_cycle_failed" }); await repo.finishRun(run.id, "failed", {}, error.message); throw error;
  }
}
  ...
  if (stale) {
    ...
    if (candidate) {
      ...
      if (!decision.blocking_thresholds.length && slot.at) {
        const plan = await readExecutionPlan(repo, slot.at, config); const planItem = plan && repo.getExecutionPlanItemForDraft ? await repo.getExecutionPlanItemForDraft(missedDraft.id).catch(() => null) : null;
        if (!plan || planItem) replacement = await repo.createAutonomySchedule({ draft_id: missedDraft.id, decision_id: null, execution_plan_item_id: planItem?.id || undefined, scheduled_for: slot.at, status: "scheduled", objective: decision.objective, reason: "recovered_after_oauth_reconnect" });
...
async function autonomyStatus() {
  ...
  const plan = await readExecutionPlan(repository, Date.now(), config); const planItems = plan && repository.listExecutionPlanItems ? await repository.listExecutionPlanItems(plan.id).catch(() => []) : [];
...
async function dailyPlanStatus() {
  const config = await operationalConfig(); const latest = await repository.latestRun("autonomy").catch(() => null); const summary = latest?.summary && typeof latest.summary === "object" ? latest.summary : null; const status = summary?.status || dailyPlan.dailyStatus({ blocker: "Daily plan has not run" });
  return { generated_at: latest?.completed_at || latest?.started_at || null, plan_date: summary?.plan?.slots?.[0]?.date_key || dailyPlan.dayKey(new Date(), config.timezone), target: status.target, published: status.published, scheduled: status.scheduled, remaining_minimum: status.remaining_minimum, next_scheduled_slot: status.next_scheduled_slot, at_risk: Boolean(status.at_risk), blocker: status.blocker || null, skipped: summary?.skipped || null };
}
...
module.exports = { discover, dailyAutonomyPlan, dailyPlanStatus, publishNext, publishApprovedDraft, approveDraft, rejectDraft, regenerateDraft, regenerateAllLegacyDrafts, markLegacyDrafts, deleteDraft, editDraft, scheduleDraft, testPost, heartbeat, engagementCheck, accountActivitySync, operationalConfig, canPublish, jaccard, backfillDrafts, backfillSkipReason, generateAndStoreDraft, autonomyDecisionCycle, autonomyPublishingCheck, autonomyPublishDiagnostic, recoverAfterOAuthReconnect, scheduledPublishingCheck, autonomyMetricsCheck, autonomyStatus, selfHealingStatus, xAccountStatus, verifyXAccount, disconnectXAccount, learningShadowCycle, socialRadarCycle, growthDirectorCycle, growthDailyBrief, growthIntelligenceCycle, monthlyExecutiveReport, refreshEditorProfile, recordEditorFeedback, discoveryHierarchy: HIERARCHY, scoreDiscoveryCandidate, selectHierarchicalCandidate, autonomy };
