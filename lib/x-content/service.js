...
const { FORMAT_LABELS, normalizeCitation, validateEditorialDraft } = require("./editorial");
...
async function generateAndStoreDraft(candidate, config, options = {}) {
  ...
  const input = candidateInput(candidate); let generated = await draftGenerator(input, config); generated = { ...generated, post_text: normalizeCitation(generated.post_text, input) }; let validation = validatePostText(generated.post_text); let editorial = validateEditorialDraft(generated, input, config.editorialThreshold); let rewrites = 0;
  while ((!editorial.ok || validation.weighted > 240) && rewrites < 2) { generated = await draftGenerator(input, config, generated.post_text); generated = { ...generated, post_text: normalizeCitation(generated.post_text, input) }; validation = validatePostText(generated.post_text); editorial = validateEditorialDraft(generated, input, config.editorialThreshold); rewrites += 1; }
  ...
  const status = validation.ok && editorial.ok && duplicateScore < 0.82 && qualityScore >= threshold ? "queued" : "rejected";
  const draft = await repo.createDraft({ candidate_id: candidate.id, text: generated.post_text, weighted_character_count: validation.weighted, raw_character_count: validation.raw, post_type: STORAGE_POST_TYPES[generated.post_type] || "practical_insight", topic_cluster: generated.topic_cluster || candidate.topic_cluster, source_references: [input.sourceUrl], confidence: generated.confidence, quality_score: qualityScore, duplicate_score: duplicateScore, mode: config.mode, status, rejection_reason: status === "rejected" ? [...validation.errors, ...editorial.errors, validation.weighted > 240 ? "Rejected after two V2 rewrites" : "", duplicateScore >= 0.82 ? "Duplicate draft" : ""].filter(Boolean).join("; ") : null, model_output: { ...generated, rewrite_attempts: rewrites, v2: { format: generated.post_type, format_label: FORMAT_LABELS[generated.post_type] || generated.post_type, source_label: input.publisher, mention_preview: editorial.mention_preview, scores: editorial.scores, target: editorial.target, soft_max: editorial.soft_max, hard_max: editorial.hard_max } } });
...
async function regenerateDraft(id) {
  ...
  const otherDrafts = (await repository.recentDrafts()).filter((row) => row.id !== draft.id && row.status !== "rejected");
  ...
  const result = await generateAndStoreDraft(candidate, config);
...
module.exports = { discover, publishNext, publishApprovedDraft, approveDraft, rejectDraft, regenerateDraft, testPost, heartbeat, engagementCheck, operationalConfig, canPublish, jaccard, backfillDrafts, backfillSkipReason, generateAndStoreDraft };
