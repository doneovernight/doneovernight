...
async function heartbeat() {
  ...
  const [discovery, publishing, drafts, publications, activityRows, activitySetting] = await Promise.all([
    repository.latestRun("discovery"), repository.latestRun("publishing"), repository.recentDrafts(), repository.publicationsToday(),
    repository.listAccountActivity(1000).catch(() => null), repository.getSetting(accountActivity.SYNC_SETTING).catch(() => null)
  ]);
...
async function engagementCheck() {
  ...
  try { const [activity, engagement, analytics] = await Promise.all([accountActivitySync(), collectEngagement({ config }), collectAnalytics()]); return { schema_pending: false, account_activity: activity, engagement, analytics }; }
  catch (error) {
...
async function autonomyPublishingCheck(options = {}) {
  ...
  try { const summary = await autonomy.processScheduled({ ...options, repository: repo, config, runId: run.id, notify: options.notify || notify }); await repo.finishRun(run.id, "completed", summary); if (summary.published) await accountActivitySync({ repository: repo }).catch(() => null); return summary; }
...
async function scheduledPublishingCheck(options = {}) {
  ...
  const summary = await processScheduled({ ...options, config, notify: options.notify || notify }); if (summary?.published) await accountActivitySync({ repository: options.repository || repository }).catch(() => null); return summary;
}
async function autonomyMetricsCheck(options = {}) {
  ...
  try { await autonomy.audit(repo, { event_type: "cycle_started", run_id: run.id, mode: config.autonomy.mode, reason: "metrics_cycle_started" }); const metrics = await autonomy.collectMetricCheckpoints({ ...options, repository: repo, config, runId: run.id }); const learning = await autonomy.runLearningCycle({ ...options, repository: repo, config, runId: run.id }); const summary = { metrics, learning }; await autonomy.audit(repo, { event_type: "cycle_completed", run_id: run.id, mode: config.autonomy.mode, reason: "metrics_cycle_completed", payload: { checkpoints: metrics.checkpoints, learning_adjusted: learning.adjusted } }); await repo.finishRun(run.id, "completed", summary); return summary; }
...
async function growthDailyBrief(options = {}) {
  ...
  try {
    ...
    const brief = growth.dailyBrief({ publications, performance, interactions, sources, schedules, editorProfile, now, timeZone: config.timezone }); const saved = await repo.saveGrowthDailyBrief(brief); const localHour = amsterdamHour(now); const deliver = options.deliver === true && localHour === 8 && !saved?.delivered_at;
...
module.exports = { discover, publishNext, publishApprovedDraft, approveDraft, rejectDraft, regenerateDraft, regenerateAllLegacyDrafts, markLegacyDrafts, deleteDraft, editDraft, scheduleDraft, testPost, heartbeat, engagementCheck, accountActivitySync, operationalConfig, canPublish, jaccard, backfillDrafts, backfillSkipReason, generateAndStoreDraft, autonomyDecisionCycle, autonomyPublishingCheck, scheduledPublishingCheck, autonomyMetricsCheck, autonomyStatus, learningShadowCycle, socialRadarCycle, growthDirectorCycle, growthDailyBrief, growthIntelligenceCycle, monthlyExecutiveReport, refreshEditorProfile, recordEditorFeedback, autonomy };
