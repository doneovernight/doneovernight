...
async function autonomyPublishDiagnostic(options = {}) {
  ...
  const schedule = failed[0] || schedules.filter((row) => row.status === "scheduled").sort((a, b) => new Date(a.scheduled_for || 0) - new Date(b.scheduled_for || 0))[0] || null;
  ...
  const connectionRow = await repo.getSetting("x_oauth2_connection").catch(() => null); let connection;
  try { const stored = JSON.parse(connectionRow?.value || "null"); connection = { present: Boolean(stored), access_token_present: Boolean(stored?.access_token), refresh_token_available: Boolean(stored?.refresh_token), scopes: Array.isArray(stored?.scopes) ? stored.scopes.filter((scope) => ["tweet.read", "tweet.write", "users.read", "offline.access"].includes(scope)) : [], expires_at: stored?.expires_at || null }; } catch { connection = { present: false, access_token_present: false, refresh_token_available: false, scopes: [], expires_at: null }; }
  let identity;
...
module.exports = { discover, publishNext, publishApprovedDraft, approveDraft, rejectDraft, regenerateDraft, regenerateAllLegacyDrafts, markLegacyDrafts, deleteDraft, editDraft, scheduleDraft, testPost, heartbeat, engagementCheck, accountActivitySync, operationalConfig, canPublish, jaccard, backfillDrafts, backfillSkipReason, generateAndStoreDraft, autonomyDecisionCycle, autonomyPublishingCheck, autonomyPublishDiagnostic, scheduledPublishingCheck, autonomyMetricsCheck, autonomyStatus, learningShadowCycle, socialRadarCycle, growthDirectorCycle, growthDailyBrief, growthIntelligenceCycle, monthlyExecutiveReport, refreshEditorProfile, recordEditorFeedback, autonomy };
