...
async function dailyAutonomyPlan(options = {}) {
  ...
  try {
    const planDate = dailyPlan.dayKey(new Date(now), config.timezone); const marker = repo.getSetting ? await repo.getSetting("x_daily_plan_date_v2").catch(() => null) : null;
    ...
    for (const slot of plan.slots) {
      ...
      result.drafts_selected += 1; let status = config.autonomy.mode === "auto" && config.autonomy.publishEnabled ? "scheduled" : "shadow";
...
async function autonomyMetricsCheck(options = {}) {
  ...
  catch (error) { await autonomy.audit(repo, { event_type: "cycle_completed", run_id: run.id, mode: config.autonomy.mode, reason: "metrics_cycle_failed" }); await selfHealing.recordIncident(repo, { component: "analytics", error, phase: "metrics_cycle", run_id: run.id, reference: run.id }); await repo.finishRun(run.id, "failed", {}, error.message); throw error; }
}
