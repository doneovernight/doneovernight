...
async function generateAndStoreDraft(candidate, config, options = {}) {
  ...
  if (status === "queued") {
    // Telegram Control is deliberately opt-in: a card is sent only for an explicitly
    // configured single admin chat/user pair, never to the general heartbeat channel.
    const telegram = require("./telegram-control"); const telegramConfig = telegram.controlConfig();
    if (!options.repository && telegramConfig.chatIds.size === 1 && telegramConfig.userIds.size === 1) {
      await telegram.reviewCard({ draft, chatId: [...telegramConfig.chatIds][0], userId: [...telegramConfig.userIds][0], config: telegramConfig, candidate: { ...candidate, ...input } }).catch(() => null);
    } else if (options.notify) await notifyFn(`DONEOVERNIGHT X: review-ready draft generated (${FORMAT_LABELS[generated.post_type] || generated.post_type}, ${validation.weighted}/240).`);
  }
...
async function growthDirectorCycle(options = {}) {
  ...
  try { const summary = await growth.runCycle({ repository: repo, config, now: options.now }); await repo.finishRun(run.id, "completed", summary); return summary; }
...
async function growthDailyBrief(options = {}) {
  ...
  const run = await repo.createRun("daily_brief");
  try {
    ...
    const summary = { published: false, brief: saved || brief, delivered: Boolean(telegram.sent), delivery: telegram.sent ? "sent" : telegram.skipped || "not_sent", attention_required: brief.attention_required, safeguards: { auto_publish: false, auto_reply: false, auto_repost: false } }; await repo.finishRun(run.id, "completed", summary); return summary;
  } catch (error) { await repo.finishRun(run.id, "failed", {}, error.message); throw error; }
}
async function growthIntelligenceCycle(options = {}) {
  ...
  try { const summary = await intelligence.run({ repository: repo, config, now: options.now }); await repo.finishRun(run.id, "completed", summary); return summary; }
  catch (error) { await repo.finishRun(run.id, "failed", {}, error.message); throw error; }
}
...
module.exports = { discover, publishNext, publishApprovedDraft, approveDraft, rejectDraft, regenerateDraft, regenerateAllLegacyDrafts, markLegacyDrafts, deleteDraft, editDraft, scheduleDraft, testPost, heartbeat, engagementCheck, operationalConfig, canPublish, jaccard, backfillDrafts, backfillSkipReason, generateAndStoreDraft, autonomyDecisionCycle, autonomyPublishingCheck, scheduledPublishingCheck, autonomyMetricsCheck, autonomyStatus, learningShadowCycle, socialRadarCycle, growthDirectorCycle, growthDailyBrief, growthIntelligenceCycle, monthlyExecutiveReport, refreshEditorProfile, recordEditorFeedback, autonomy };
