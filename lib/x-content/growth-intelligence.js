...
function health({ performance = [], drafts = [], feedback = [], publications = [], accountActivity = [] } = {}) {
  ...
  const timeline = Array.isArray(accountActivity) ? accountActivity : [];
  const originals = timeline.filter((row) => !row.is_reply && !row.is_repost && row.is_currently_visible !== false);
  const publicationIds = new Set(publications.map((row) => String(row.x_post_id || "")).filter(Boolean));
  return { follower_velocity: null, engagement_velocity: normalized, reach_velocity: average(performance, "views"), authority_score: authority, educational_score: clamp(average(drafts.map((draft) => ({ value: draft.model_output?.v2?.scores?.educational || 0 })), "value")), brand_consistency: clamp(average(drafts.map((draft) => ({ value: draft.model_output?.v2?.scores?.brand || 0 })), "value")), community_health: null, trust_score: trust, posting_consistency: publications.length ? 1 : 0, content_diversity: diversity, timeline: { known_original_posts: originals.length, known_manual_posts: originals.filter((row) => String(row.publication_origin).startsWith("manual")).length, agent_publications_missing_from_timeline: [...publicationIds].filter((id) => !new Set(originals.map((row) => String(row.x_post_id))).has(id)).length }, sample_sizes: { performance: performance.length, drafts: drafts.length, feedback: feedback.length, account_activity: timeline.length } };
}
...
async function run({ repository, config = {}, now = Date.now(), runId = null, audit = async () => null }) {
  const [drafts, publications, performance, feedback, radarItems, accountActivity] = await Promise.all([repository.listDrafts(), repository.listPublications(300), repository.listPerformanceMemory(300), repository.listEditorFeedback(300), repository.listRadarItems(200), repository.listAccountActivity ? repository.listAccountActivity(1000).catch(() => []) : []]);
  const accountHealth = health({ performance, drafts, feedback, publications, accountActivity }); const memories = [];
