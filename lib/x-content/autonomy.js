...
async function runAutonomyCycle(options = {}) {
  ...
  const candidates = drafts.filter((row) => row.status === "queued" && !isLegacyDraft(row) && (!canonicalPlan || planByDraft.has(String(row.id)))).sort((left, right) => predictionRank(right, publications, draftsById, now) - predictionRank(left, publications, draftsById, now));
  ...
  for (const draft of candidates) {
    ...
    const slot = decision.would_auto_approve ? nextSlot({ from: now, config, publications: [...publications, ...planned], draftsById, draft, candidate }) : null;
    ...
    if (existing && ["scheduled", "due", "publishing", "delayed"].includes(existing.status)) {
      if (planItem && repo.updateExecutionPlanItem) await repo.updateExecutionPlanItem(planItem.id, { decision_id: persisted?.id || null, gate_audit_id: persistedAudit?.id || null, schedule_id: existing.id, intended_at: existing.scheduled_for || slot.at, lifecycle_status: "scheduled" });
      result.scheduled.push({ draft_id: draft.id, scheduled_for: existing.scheduled_for || slot.at, status: existing.status, objective: decision.objective, plan_item_id: planItem?.id || null, idempotent: true });
      ...
      if (canActivateSchedule) {
        await repo.updateDraft(draft.id, { status: "approved", approved_at: new Date(now).toISOString() });
        await audit(repo, { event_type: "draft_auto_approved", run_id: runId, draft_id: draft.id, mode: config.autonomy.mode, reason: "all_hard_gates_pass", created_at: new Date(now).toISOString() });
        const schedule = await repo.createAutonomySchedule({ draft_id: draft.id, decision_id: persisted?.id || null, execution_plan_item_id: planItem.id, scheduled_for: slot.at, status: "scheduled", objective: decision.objective, reason: "canonical_execution_plan" });
        await repo.updateExecutionPlanItem(planItem.id, { gate_audit_id: persistedAudit?.id || null, decision_id: persisted?.id || null, schedule_id: schedule?.id || null, intended_at: slot.at, lifecycle_status: schedule ? "scheduled" : "evaluated", blocker_code: schedule ? null : "schedule_persistence_failed", blocker_reason: schedule ? null : "Canonical schedule could not be persisted" });
        if (schedule) { result.scheduled.push({ draft_id: draft.id, scheduled_for: slot.at, status: "scheduled", objective: decision.objective, plan_item_id: planItem.id }); await audit(repo, { event_type: "schedule_proposed", run_id: runId, draft_id: draft.id, schedule_id: schedule.id, mode: config.autonomy.mode, reason: "canonical_execution_plan", created_at: new Date(now).toISOString() }); }
...
async function processScheduled(options = {}) {
  ...
  try {
    ...
    if (planItem && repo.updateExecutionPlanItem) await repo.updateExecutionPlanItem(planItem.id, { lifecycle_status: "publishing", blocker_code: null, blocker_reason: null });
...
module.exports = { CHECKPOINTS, OBJECTIVES, LEARNING_POST_LIMIT, PAUSE_KEY, SAFE_STOP_KEY, SCHEDULE_STATUSES, activateLearningVersion, audit, auditMode, cadenceBlocks, collectMetricCheckpoints, evaluateDraft, forceHumanReview, isWithinAutonomyWindow, learningStatus, nextSlot, objectiveFor, parseWindows, processScheduled, requiredAudit, runAutonomyCycle, runLearningCycle, setPause, cancelSchedule, revertLearningVersion, strategicMixPenalty, scheduleGraceMs };
