...
async function collectMetricCheckpoints(options = {}) {
  const repo = options.repository || repository; const client = options.xClient || xClient; const now = options.now || Date.now(); const publications = await repo.listPublishedPublications(); let collected = 0;
  for (const publication of publications) { const publishedAt = asDate(publication.published_at); if (!publishedAt) continue; for (const hours of CHECKPOINTS) { if (now < publishedAt.getTime() + hours * 3600000) continue; const metrics = await client.getPostMetrics(publication.x_post_id); const data = metrics.data?.data || {}; const values = data.public_metrics || {}; const snapshot = await repo.createMetricCheckpoint({ publication_id: publication.id, checkpoint_hours: hours, due_at: new Date(publishedAt.getTime() + hours * 3600000).toISOString(), metrics: values, normalized_performance: Number(values.impression_count || 0) ? (Number(values.like_count || 0) + Number(values.retweet_count || 0) + Number(values.reply_count || 0)) / Number(values.impression_count || 1) : null }); if (snapshot) collected += 1; }
  }
  return { checkpoints: collected, eligible_publications: publications.length };
}
