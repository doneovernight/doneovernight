...
async function runAutonomyCycle(options = {}) {
  ...
  for (const draft of drafts.filter((row) => row.status === "queued" && !isLegacyDraft(row))) {
    ...
    if (!existing) {
      ...
      if (config.autonomy.mode === "auto" && config.autonomy.publishEnabled && !runtimeState.paused && !runtimeState.safeStop) { await repo.updateDraft(draft.id, { status: "approved", approved_at: new Date(now).toISOString() }); await audit(repo, { event_type: "draft_auto_approved", run_id: runId, draft_id: draft.id, mode: config.autonomy.mode, reason: "all_hard_gates_pass", created_at: new Date(now).toISOString() }); }
...
async function processScheduled(options = {}) {
  ...
  if (blocks.length || draft.status !== "approved") { const reason = blocks.length ? blocks.join(",") : "manual_approval_required"; await repo.updateAutonomySchedule(due.id, { status: "cancelled", reason }); await audit(repo, { event_type: "schedule_cancelled", run_id: runId, draft_id: draft?.id || null, schedule_id: due.id, mode: config.autonomy.mode, reason, created_at: new Date(now).toISOString() }); await audit(repo, { event_type: "publish_skipped", run_id: runId, draft_id: draft?.id || null, schedule_id: due.id, mode: config.autonomy.mode, reason, created_at: new Date(now).toISOString() }); await audit(repo, { event_type: "cycle_completed", run_id: runId, mode: config.autonomy.mode, reason, created_at: new Date(now).toISOString() }); return { ...result, skipped: reason }; }
  ...
  try {
    ...
    const url = `https://x.com/${identity.username}/status/${xPostId}`; await repo.updatePublication(draft.id, { status: "published", x_post_id: xPostId, x_post_url: url, published_at: new Date(now).toISOString(), x_response_status: 201 }); await repo.updateDraft(draft.id, { status: "published", published_at: new Date(now).toISOString(), x_post_id: xPostId, x_post_url: url }); await repo.updateAutonomySchedule(due.id, { status: "published" }); await audit(repo, { event_type: "publish_succeeded", run_id: runId, draft_id: draft.id, publication_id: publication.id || null, schedule_id: due.id, mode: config.autonomy.mode, reason: "x_post_created", payload: { x_post_id: xPostId }, created_at: new Date(now).toISOString() }); await audit(repo, { event_type: "cycle_completed", run_id: runId, mode: config.autonomy.mode, reason: "publish_succeeded", created_at: new Date(now).toISOString() }); return { ...result, processed: 1, published: true, url };
  } catch (error) { const reason = error.category === "authentication" ? "oauth_failure" : "x_publish_failure"; await repo.setSetting(SAFE_STOP_KEY, "true"); if (publication?.id) await repo.updatePublication(draft.id, { status: "failed" }).catch(() => null); await repo.updateAutonomySchedule(due.id, { status: "cancelled", reason: "x_publish_failure" }); await audit(repo, { event_type: "publish_failed", run_id: runId, draft_id: draft?.id || null, publication_id: publication?.id || null, schedule_id: due.id, mode: config.autonomy.mode, reason, payload: { category: error.category || "unknown", code: error.code || null }, created_at: new Date(now).toISOString() }); await audit(repo, { event_type: "cycle_completed", run_id: runId, mode: config.autonomy.mode, reason: "safe_stop_activated", created_at: new Date(now).toISOString() }); await (options.notify || (async () => ({})))(error.category === "authentication" ? "DONEOVERNIGHT X: OAuth failure. Autonomous publishing is stopped." : "DONEOVERNIGHT X: autonomous publishing stopped after a publish failure.").catch(() => {}); return { ...result, processed: 1, skipped: "X failure activated safe stop" }; }
}
...
module.exports = { CHECKPOINTS, OBJECTIVES, PAUSE_KEY, SAFE_STOP_KEY, activateLearningVersion, audit, auditMode, cadenceBlocks, collectMetricCheckpoints, evaluateDraft, forceHumanReview, nextSlot, objectiveFor, parseWindows, processScheduled, requiredAudit, runAutonomyCycle, runLearningCycle, setPause, cancelSchedule, revertLearningVersion, strategicMixPenalty };
