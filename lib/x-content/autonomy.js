...
async function runAutonomyCycle(options = {}) {
  ...
  const [drafts, publications, schedules, runtimeState, learning, planItems] = await Promise.all([repo.listDrafts(), repo.listPublishedPublications(), repo.listAutonomySchedules(), state(repo), learningStatus(repo), typeof repo.listExecutionPlanItems === "function" ? repo.listExecutionPlanItems(null).catch(() => null) : null]);
