...
async function runAutonomyCycle(options = {}) {
  ...
  const candidates = drafts.filter((row) => row.status === "queued" && !isLegacyDraft(row)).sort((left, right) => predictionRank(right, publications, draftsById, now) - predictionRank(left, publications, draftsById, now));
  ...
  for (const draft of candidates) {
    ...
    auditRows.push(audited); await gateAudit.persist(repo, audited); result.gate_audit = gateAudit.summarize(auditRows);
