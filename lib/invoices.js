...
async function updateInvoiceWorkspace(invoice = {}, workspace = {}, workspaceUrl = "") {
  if (!invoice?.id) return invoice;
  const nextInvoice = {
    ...invoice,
    workspace_id: clean(workspace.id || invoice.workspace_id),
    workspace_url: clean(workspaceUrl || invoice.workspace_url)
  };
  await supabaseFetch(`${PAYMENT_RECORDS_TABLE}?id=eq.${encodeURIComponent(invoice.id)}`, {
    method: "PATCH",
    headers: { Prefer: "return=minimal" },
    body: JSON.stringify({
      workspace_id: nextInvoice.workspace_id,
      invoice_snapshot: nextInvoice
    })
  });
  return nextInvoice;
}
