...
async function findOrCreateWorkspace(task = {}, input = {}) {
  ...
  if (existing?.id) {
    ...
    const rows = await workspaceSupabaseWriteAttempts(`portal_requests?id=eq.${encodeURIComponent(existing.id)}`, {
      ...
      fullPayload,
      pickPayload(fullPayload, ["name", "company", "username", "access_key", "status", "raw_payload", "updated_at"]),
      pickPayload(fullPayload, ["name", "username", "access_key", "status", "raw_payload", "updated_at"]),
      pickPayload(fullPayload, ["status", "raw_payload", "updated_at"]),
      pickPayload(fullPayload, ["raw_payload", "updated_at"])
    ], {
      table: "portal_requests",
      operation: "PATCH",
      lookup_column: "id"
  ...
  const rows = await workspaceSupabaseWriteAttempts("portal_requests", {
    ...
    fullPayload,
    pickPayload(fullPayload, ["email", "name", "username", "company", "workspace_slug", "access_key", "credentials_issued_at", "status", "source", "signup_method", "intake_task_id", "raw_payload", "created_at", "updated_at"]),
    pickPayload(fullPayload, ["email", "name", "username", "company", "access_key", "status", "raw_payload", "created_at", "updated_at"]),
    pickPayload(fullPayload, ["email", "name", "username", "access_key", "status", "raw_payload", "created_at", "updated_at"]),
    pickPayload(fullPayload, ["email", "name", "username", "raw_payload", "created_at", "updated_at"])
  ], {
    table: "portal_requests",
    operation: "POST"
  });
  ...
  if (alreadyActive) {
    ...
    return {
      ...
      session: {
        token: "",
        expiresAt: "",
        path: workspaceSlug ? `/workspace/@${workspaceSlug}` : "/workspace",
        url: workspaceSlug ? `https://portal.doneovernight.com/workspace/@${workspaceSlug}` : "https://portal.doneovernight.com/workspace"
      },
      payment,
      invoice: {
        configured: true,
