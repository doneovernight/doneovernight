...
function normalizeInvoiceInput(input = {}, booking = {}) {
  const customerName = clean(input.customer_name || input.customerName || booking.name);
  const customerEmail = clean(input.customer_email || input.customerEmail || booking.email).toLowerCase();
  const customerCompany = clean(input.customer_company || input.customerCompany || booking.brandCompany);
  ...
  return {
    ...
    customer_details: {
      address: clean(input.customer_address || input.customerAddress),
      vat_number: clean(input.customer_vat_number || input.customerVatNumber)
    },
...
function summarizeInvoices(invoices = []) {
  ...
  return {
    ...
    accountingRule: "paid_subtotal_excluding_vat"
  };
}
