...
async function loadRuntimeData(profile, isOwner) {
  ...
  return {
    ...
    assignments: activeAssignments.slice(0, 12).map((assignment) => ({
      ...
      task_title: assignment.task_title || "Assigned task",
      task_type: assignment.task_type || "Execution",
      operational_state: sentenceCase(assignment.operational_state || assignment.status, "Awaiting operator"),
      ...
      due_label: assignment.due_at ? formatRuntimeTime(assignment.due_at, timezone) : "No delivery window",
    ...
    assigned_clients: [
      ...
      return {
      workspace_slug: assignment.client_name || assignment.workspace_slug || "workspace",
      ...
      ...activeRelationships
        ...
        .map((relationship) => ({
          workspace_slug: relationship.client_name || relationship.workspace_slug || relationship.client_email || "Linked client",
    ...
    tasks: [
      ...activeAssignments.map((assignment) => ({
        id: assignment.id || assignment.task_id || "",
        title: assignment.task_title || "Assigned task",
        status: sentenceCase(assignment.operational_state || "Awaiting operator"),
        deadline: assignment.due_at ? formatRuntimeTime(assignment.due_at, timezone) : "No deadline",
        workspace_slug: assignment.workspace_slug || "",
        type: assignment.task_type || "Execution"
      })),
      ...scopedTasks.map((task) => ({
      id: task.task_id || task.id,
      title: task.task_summary || task.task_description || "Assigned task",
      status: task.status || "pending",
      deadline: task.deadline ? formatRuntimeTime(task.deadline, timezone) : "No deadline",
      workspace_slug: task.workspace_slug || task.raw_payload?.workspace_slug || ""
      }))
    ].slice(0, 6),
    messages: [
      ...activity.map((item) => ({
