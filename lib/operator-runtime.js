...
const { resolveOperatorAvailability } = require("./operator-availability");
...
function relationshipReferences(relationship = {}) {
  const raw = relationship.raw_payload && typeof relationship.raw_payload === "object" ? relationship.raw_payload : {};
  return [
...
function relationshipSourceLabel(value = "") {
  const normalized = clean(value).toLowerCase();
  if (normalized === "operator_referral") return "Operator referral";
  if (normalized === "direct_assignment") return "Direct assignment";
  if (normalized === "operator_existing_account") return "Existing account connected";
  if (normalized === "operator_google_signin") return "Google through operator";
  if (normalized === "client_onboarding") return "Client onboarding";
  return sentenceCase(normalized || "operator referral");
}
...
async function loadRuntimeData(profile, isOwner) {
  ...
  const unreadActivity = activity.filter((item) => {
    ...
    if (rawPayload.message_type === "OPERATOR_SUPPORT_MESSAGE" || rawPayload.message_type === "OPERATOR_SUPPORT_REQUEST") return false;
  ...
  const supportMessages = activity
    .filter((item) => {
      const rawPayload = item.raw_payload && typeof item.raw_payload === "object" ? item.raw_payload : {};
    ...
    .map((item) => {
      ...
      return {
        ...
        conversation_id: item.conversation_id || rawPayload.conversation_id || rawPayload.thread_id || `operator_support:${canonicalProfileHandle(profile)}`,
        sender_role: role,
        ...
        task_reference: item.task_reference || item.don_reference || item.task_id || rawPayload.task_reference || "",
        title: item.title || (role === "admin" ? "HQ reply" : "Support message"),
        status: item.read_at || item.operator_read_at || rawPayload.operator_read_at || rawPayload.viewed_at ? "Read" : role === "admin" ? "Unread" : "Sent",
        detail: clean(item.body || item.message || item.detail).slice(0, 360) || "Support message recorded.",
        created_at_raw: item.created_at || rawPayload.sent_at || "",
        created_at: formatRuntimeTime(item.created_at || rawPayload.sent_at, timezone),
        unread_for_operator: role === "admin" && (item.unread_for_operator === true || rawPayload.unread_for_operator === true) && !item.operator_read_at && !rawPayload.operator_read_at,
        resolved_at: item.resolved_at || rawPayload.resolved_at || ""
      };
    })
    .sort((a, b) => Date.parse(a.created_at_raw || 0) - Date.parse(b.created_at_raw || 0));
  const clientPortfolio = activeRelationships.map((relationship) => {
    const slug = clean(relationship.workspace_slug);
    const references = relationshipReferences(relationship);
    const clientRawTasks = scopedTasks.filter((task) => {
      const taskSlug = taskWorkspaceSlug(task);
      const reference = taskReferenceOf(task);
      return (slug && taskSlug === slug) || (reference && references.includes(reference));
    });
    const clientTasks = clientRawTasks.map((task) => summarizeTaskForOperator(task, { timezone, messages: scopedMessages, activity }));
    const latestRawTask = clientRawTasks
      .slice()
      .sort((a, b) => Date.parse(b.updated_at || b.created_at || 0) - Date.parse(a.updated_at || a.created_at || 0))[0] || null;
    const latestTask = latestRawTask ? summarizeTaskForOperator(latestRawTask, { timezone, messages: scopedMessages, activity }) : null;
    const clientMessages = scopedMessages.filter((message) => slug && clean(message.workspace_slug) === slug);
    const unreadUpdates = clientMessages.filter((message) => {
      const role = clean(message.author_role || message.sender_role || message.role).toLowerCase();
      return role && role !== "operator" && !message.read_at;
    }).length;
    const raw = relationship.raw_payload && typeof relationship.raw_payload === "object" ? relationship.raw_payload : {};
    const source = relationship.connection_source || relationship.source || raw.source || "operator_referral";
    return {
      id: clean(relationship.id || relationship.portal_request_id || relationship.client_id || slug || relationship.client_email),
      client_name: clean(relationship.client_name || raw.client_name || relationship.client_email || slug) || "Connected client",
      client_email: clean(relationship.client_email || raw.client_email),
      workspace_slug: slug,
      connected_date: relationship.linked_at || raw.connected_at || relationship.created_at || relationship.updated_at || "",
      connected_date_label: relationship.linked_at || raw.connected_at || relationship.created_at ? formatRuntimeTime(relationship.linked_at || raw.connected_at || relationship.created_at, timezone) : "No date",
      source,
      source_label: relationshipSourceLabel(source),
      relationship_status: sentenceCase(relationship.relationship_status || "active"),
      don_references: references,
      task_count: clientTasks.length,
      latest_task_reference: latestTask?.id || references[0] || "",
      latest_task_status: latestTask?.status || "No active task",
      latest_movement: latestRawTask?.updated_at || latestRawTask?.created_at ? `Task movement · ${relativeActivity(latestRawTask.updated_at || latestRawTask.created_at)}` : relationship.updated_at ? `Relationship updated · ${relativeActivity(relationship.updated_at)}` : "Connected",
      unread_client_updates: unreadUpdates,
      workspace_url: slug ? `https://portal.doneovernight.com/@${encodeURIComponent(slug)}` : ""
    };
  }).sort((a, b) => Date.parse(b.connected_date || 0) - Date.parse(a.connected_date || 0));
  ...
  return {
    ...
    state: {
      ...
      unread_support_messages: supportMessages.filter((item) => item.unread_for_operator).length,
    ...
    assigned_clients: [
      ...
      ...activeRelationships
        ...
        .map((relationship) => ({
          ...
          don_references: [
            ...String(relationship.task_id || relationship.task_reference || relationship.don_reference || "").split(/[\s,]+/),
            ...(Array.isArray(relationship.raw_payload?.task_references) ? relationship.raw_payload.task_references : []),
            ...(Array.isArray(relationship.raw_payload?.don_references) ? relationship.raw_payload.don_references : [])
          ].map(clean).filter(Boolean),
          source: relationship.connection_source || relationship.source || relationship.raw_payload?.source || "operator_referral",
          relationship_created_at: relationship.linked_at || relationship.created_at || relationship.updated_at || "",
    ...
    messages: [
      ...activity.filter((item) => {
        const rawPayload = item.raw_payload && typeof item.raw_payload === "object" ? item.raw_payload : {};
        return rawPayload.message_type !== "OPERATOR_SUPPORT_MESSAGE"
          && rawPayload.message_type !== "OPERATOR_SUPPORT_REQUEST"
          && item.activity_type !== "operator_support_message"
    ...
    support_messages: supportMessages.slice(-40),
