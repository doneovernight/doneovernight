...
const DOCUMENT_PLACEHOLDER_COPY_PATTERN = /\[\s*(?:add approved business copy|voeg goedgekeurde bedrijfstekst toe)\.?\s*\]/i;

function assertPublishableBusinessDocument(document = {}) {
  const copy = `${clean(document.title)}\n${clean(document.body)}`;
  if (DOCUMENT_PLACEHOLDER_COPY_PATTERN.test(copy)) {
    throw businessError(
      "Replace all placeholder copy with approved business content before publishing.",
      "DOCUMENT_PLACEHOLDER_COPY_NOT_PUBLISHABLE",
      422
    );
  }
  return true;
...
function isSystemAcceptanceEvidenceRecord(document = {}) {
  if (clean(document.status).toLowerCase() !== "archived") return false;
  const metadata = clean(document.internal_notes).toLowerCase();
  if (metadata.includes("system_acceptance_evidence")) return true;
  const controlled = metadata.includes("controlled") || metadata.includes("fixture") || metadata.includes("test");
  const acceptance = metadata.includes("acceptance") || metadata.includes("policy") && metadata.includes("evidence");
  return controlled && acceptance;
}

function classifyBusinessBundleRecords({ documents = [], workflows = [], policies = [], acceptances = [], invoiceDocuments = [] } = {}) {
  const classifiedDocuments = documents.map((document) => ({
    ...document,
    system_evidence: isSystemAcceptanceEvidenceRecord(document)
  }));
  const evidenceDocumentIds = new Set(classifiedDocuments.filter((document) => document.system_evidence).map((document) => document.id));
  const visiblePolicies = policies.filter((policy) => !evidenceDocumentIds.has(policy.document_id));
  const visiblePolicyIds = new Set(visiblePolicies.map((policy) => policy.id));
  return {
    documents: classifiedDocuments,
    documentWorkflows: workflows.filter((workflow) => !evidenceDocumentIds.has(workflow.document_id)),
    policies: visiblePolicies,
    policyAcceptances: acceptances.filter((acceptance) => visiblePolicyIds.has(acceptance.policy_id)),
    invoiceDocuments: invoiceDocuments.filter((link) => !evidenceDocumentIds.has(link.document_id))
  };
}

function assertMutableBusinessDocument(document) {
  if (isSystemAcceptanceEvidenceRecord(document)) {
    throw businessError("System acceptance evidence is immutable", "SYSTEM_ACCEPTANCE_EVIDENCE_IMMUTABLE", 409);
  }
}
...
async function getBusinessBundle(context) {
  ...
  return {
    ...
    ...classifyBusinessBundleRecords({ documents, workflows, policies, acceptances, invoiceDocuments })
  };
}
...
async function saveDocument(context, input) {
  ...
  assertMutableBusinessDocument(existing);
...
async function publishDocument(context, input) {
  ...
  assertMutableBusinessDocument(document);
  assertPublishableBusinessDocument(document);
...
async function duplicateDocument(context, input) {
  ...
  assertMutableBusinessDocument(source);
  return createScopedRecord(context, "document", {
...
async function savePolicy(context, input, { create = false } = {}) {
  ...
  if (!document || document.status === "archived" || isSystemAcceptanceEvidenceRecord(document)) throw businessError("Policy document is not available in this workspace", "POLICY_DOCUMENT_NOT_FOUND", 404);
...
async function readPublicBookingPolicies(workspaceId) {
  ...
  const documentMap = new Map((Array.isArray(documents) ? documents : []).filter((item) => !isSystemAcceptanceEvidenceRecord(item)).map((item) => [item.id, item]));
...
async function resolveInvoiceDocuments(context, requestedDocumentIds) {
  ...
  const selected = documents.filter((document) => selectedIds.has(document.id) && !isSystemAcceptanceEvidenceRecord(document)).map((document) => ({
    document,
