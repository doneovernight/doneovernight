...
function getDispatchTelegramConfig() {
  return {
    ...
    chatId: clean(process.env.HEARTBEAT_TELEGRAM_CHAT_ID)
  };
}
...
async function notifyDispatchSignup(contact) {
  ...
  const telegram = await sendTelegramMessage({
    ...getDispatchTelegramConfig(),
    text: telegramMessage
  });
  const delivered = telegram.sent === true;

  return {
    configured: result.attempted > 0 || telegram.provider === "bot_api",
    delivered,
    reason: delivered ? "delivered" : (result.attempted ? "failed" : telegram.reason || "not_configured"),
