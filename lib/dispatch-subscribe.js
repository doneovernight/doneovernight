...
function getDispatchOperatorUrls() {
...
function getDispatchSource(contact) {
  ...
  if (hostname.startsWith("ask.")) return "ask";
  if (hostname.startsWith("start.")) return "start";
...
async function notifyDispatchSignup(contact) {
  ...
  const payload = {
    ...
    event_type: "dispatch_operator_notification",
    notification_type: "dispatch_signup",
    type: "dispatch_signup",
    workflow_version: "dispatch_operator_notification_v1",
    timestamp: created,
    email: contact.email,
    client_email: contact.email,
    source,
  ...
  const result = await dispatchWebhook({
    tag: "[DISPATCH_OPERATOR]",
    event: payload.event,
    urls: getDispatchOperatorUrls(),
    payload,
    timeoutMs: DISPATCH_TIMEOUT_MS
  });
  ...
  return {
    configured: result.attempted > 0,
    delivered,
    reason: delivered ? "delivered" : (result.attempted ? "failed" : "not_configured"),
    provider: result.attempted ? "webhook" : "none",
    status: result
  };
}
...
async function subscribeToDispatch(input = {}) {
  ...
  try {
    operatorNotification = await notifyDispatchSignup(contact);
  } catch (error) {
    console.warn(`Dispatch operator notification warning: ${error.message}`);
    operatorNotification = {
      configured: true,
      delivered: false,
      reason: "failed",
      provider: "webhook",
      error: "DISPATCH_OPERATOR_NOTIFICATION_FAILED"
    };
  }
