...
function dispatchRouteMetadata(overrides = {}) {
  const doneovernightChatId = clean(process.env.DONEOVERNIGHT_CHAT_ID);
  const heartbeatChatId = clean(process.env.HEARTBEAT_TELEGRAM_CHAT_ID);
  return {
    destinationType: "not_configured",
    chatEnvUsed: null,
    tokenEnvUsed: null,
    webhookEnvUsed: null,
    doneovernightChatEqualsHeartbeatChat: Boolean(doneovernightChatId && heartbeatChatId && doneovernightChatId === heartbeatChatId),
    ...overrides
  };
}
...
async function notifyDispatchSignup(contact) {
  ...
  if (result.attempted === 0 && telegramConfig.botToken && telegramConfig.chatId) {
    ...
    return {
      configured: true,
      delivered,
      reason: delivered ? "delivered" : telegram.reason || "failed",
      provider: delivered || telegram.provider === "bot_api" ? "telegram_bot" : "none",
      ...dispatchRouteMetadata({
        destinationType: "doneovernight_bot",
