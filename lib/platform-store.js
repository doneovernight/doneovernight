...
async function saveLiveStatus(input = {}) {
  ...
  const progressText = clean(input.current_progress || input.currentProgress || input.progress);
  const record = {
    current_build: clean(input.current_build || input.currentBuild),
    current_operator: clean(input.current_operator || input.currentOperator),
    current_client: clean(input.current_project || input.currentProject || input.current_client || input.currentClient),
    current_repository: clean(input.current_repository || input.currentRepository || input.repository),
    current_branch: clean(input.current_branch || input.currentBranch),
    current_commit: clean(input.current_commit || input.currentCommit),
    latest_deployment: clean(input.latest_deployment || input.latestDeployment),
    heartbeat: clean(input.heartbeat),
    estimated_completion: clean(input.estimated_completion || input.estimatedCompletion),
    current_focus: clean(input.current_focus || input.currentFocus),
    current_progress: progressText,
    progress_percentage: Number.isFinite(progressNumber) ? Math.max(0, Math.min(100, Math.round(progressNumber))) : 0,
    repository_status: clean(input.repository_status || input.repositoryStatus),
    recent_activity: asArray(input.recent_activity || input.recentActivity),
    latest_wins: asArray(input.latest_wins || input.latestWins),
    recently_finished: asArray(input.recently_finished || input.recentlyFinished),
    upcoming_builds: asArray(input.upcoming_builds || input.upcomingBuilds),
    placeholder: false,
