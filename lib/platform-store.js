...
const PLATFORM_TABLES = {
  ...
  builderIdentities: "builder_identities",
  walletPasses: "wallet_passes"
};

const RESOURCE_INTEREST_OPTIONAL_COLUMNS = [
  "product",
  "category",
  "access",
  "builder_number",
...
function normalizeWalletStatus(value = "") {
  const status = clean(value).toLowerCase().replace(/[\s_-]+/g, "_");
  const allowed = new Set(["prepared", "issued", "downloaded", "active", "revoked", "expired"]);
  if (allowed.has(status)) return status;
  if (status.includes("download")) return "downloaded";
  if (status.includes("active")) return "active";
  if (status.includes("revoke")) return "revoked";
  if (status.includes("expire")) return "expired";
  if (status.includes("prepare")) return "prepared";
  return "issued";
}
...
function tableUnavailableResult(error = {}, reason = "table_not_ready") {
  const unavailable = platformUnavailable(error);
  return {
    ...unavailable,
    reason: unavailable.reason === "table_not_ready" ? reason : unavailable.reason
  };
}
...
async function saveResourceInterest(input = {}) {
  ...
  const record = {
    ...
    builder_number: clean(input.builder_number || input.builderNumber),
    selected_language: language.selected_language,
    browser_language: language.browser_language,
    detected_content_language: language.detected_content_language,
    email_language: language.email_language,
    ...
    raw_payload: {
      ...
      source_page: clean(input.source_page || input.sourcePage || input.page)
    },
  ...
  try {
    await safeWrite(PLATFORM_TABLES.resourceInterest, {
      ...
      body: JSON.stringify(record)
    }, [...LANGUAGE_COLUMNS, ...RESOURCE_INTEREST_OPTIONAL_COLUMNS]);
    return platformResult(true);
  } catch (error) {
    return platformUnavailable(error);
  }
...
async function issueBuilderIdentity(input = {}) {
  const journeyId = normalizeJourneyId(input);
  if (!journeyId) return { configured: true, saved: false, status: "skipped", reason: "missing_journey_id" };
  const language = languageFields(input);
  const record = omitEmpty({
    journey_id: journeyId,
    builder_type: clean(input.builder_type || input.builderType),
    status: clean(input.status) || "Founding Builder",
    selected_language: language.selected_language,
    browser_language: language.browser_language,
    detected_content_language: language.detected_content_language,
    email_language: language.email_language,
    identity_payload: input.identity_payload && typeof input.identity_payload === "object" ? input.identity_payload : input,
    updated_at: new Date().toISOString()
  });
  try {
    const rows = await safeWrite(`${PLATFORM_TABLES.builderIdentities}?on_conflict=journey_id`, {
      method: "POST",
    ...
    return {
      ...platformResult(true),
      identity,
      builder_number: identity?.builder_number || null
    };
  } catch (error) {
    return tableUnavailableResult(error, "builder_identity_table_not_ready");
  }
}

async function saveWalletPass(input = {}) {
  const passKind = clean(input.pass_kind || input.passKind || input.kind || input.type) || "builder";
  const provider = clean(input.provider) || "apple";
  const journeyId = normalizeJourneyId(input);
  const serialNumber = clean(input.serial_number || input.serialNumber);
  const record = omitEmpty({
    pass_kind: passKind,
    provider,
    journey_id: journeyId,
    builder_number: input.builder_number ? asNumber(input.builder_number, 0) : undefined,
    founder_id: clean(input.founder_id || input.founderId),
    serial_number: serialNumber,
    status: normalizeWalletStatus(input.status || "issued"),
    signed: asBoolean(input.signed),
    downloaded_at: clean(input.downloaded_at || input.downloadedAt) || null,
    activated_at: clean(input.activated_at || input.activatedAt) || null,
    payload: input.payload && typeof input.payload === "object" ? input.payload : {},
    updated_at: new Date().toISOString()
  });
  try {
    const rows = await safeSupabase(PLATFORM_TABLES.walletPasses, {
      method: "POST",
      headers: { Prefer: "return=representation" },
      body: JSON.stringify(record)
    });
    return { ...platformResult(true), pass: Array.isArray(rows) ? rows[0] : record };
  } catch (error) {
    if (serialNumber && (error.statusCode === 409 || String(error.detail || "").toLowerCase().includes("duplicate"))) {
      try {
        const rows = await safeSupabase(`${PLATFORM_TABLES.walletPasses}?provider=eq.${encodeURIComponent(provider)}&serial_number=eq.${encodeURIComponent(serialNumber)}`, {
          method: "PATCH",
          headers: { Prefer: "return=representation" },
          body: JSON.stringify(record)
        });
        ...
        return tableUnavailableResult(patchError, "wallet_pass_table_not_ready");
      }
    }
    return tableUnavailableResult(error, "wallet_pass_table_not_ready");
  }
}
...
async function getPlatformSnapshot() {
  ...
  const [
    ...
    listRows(PLATFORM_TABLES.builderIdentities, "select=*&order=builder_number.desc&limit=80"),
    listRows(PLATFORM_TABLES.walletPasses, "select=*&order=created_at.desc&limit=80")
  ]);
  ...
  return {
    ...
    builder_identities: builderIdentities,
