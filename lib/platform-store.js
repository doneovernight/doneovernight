...
async function saveWalletPass(input = {}) {
  ...
  try {
    const path = serialNumber
      ? `${PLATFORM_TABLES.walletPasses}?on_conflict=provider,serial_number`
      : PLATFORM_TABLES.walletPasses;
    const rows = await safeSupabase(path, {
      method: "POST",
      headers: { Prefer: serialNumber ? "resolution=merge-duplicates,return=representation" : "return=representation" },
      body: JSON.stringify(record)
    });
