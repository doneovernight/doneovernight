...
async function resolveLiveStatus(slug) {
  ...
  const username = normalizeUsername(creator.tiktok_live_username || creator.username);
  const manualLiveRequested = bool(creator.manual_live_fallback_enabled, false) && bool(creator.live_status, false);

  if (manualLiveRequested) {
    cachedStatus = null;
  } else if (cachedStatus && cachedStatus.key === cacheKey && isCacheablePayload(cachedStatus.payload) && Date.now() - cachedStatus.time < LIVE_CACHE_TTL_MS) {
  ...
  if (manualLiveRequested && (!payload || payload.isLive !== true || payload.confirmed !== true)) {
    payload = manualFallback(creator, "manual-fallback");
  }
