...
const DEFAULT_DOONIA_SETTINGS = {
  ...
  live_url: "https://www.tiktok.com/@doonia.j/live",
...
function defaultCreator(value = "mosyaamosya") {
  return DEFAULT_CREATOR_SETTINGS_BY_SLUG[normalizeSlug(value)] || DEFAULT_MINA_SETTINGS;
}
...
function normalizeCreator(row = {}) {
  ...
  return {
    ...defaults,
    ...row,
    username: normalizeUsername(row.username || defaults.username),
    slug: normalizeSlug(row.slug || defaults.slug),
    tiktok_url: clean(row.tiktok_url) || defaults.tiktok_url,
    live_url: clean(row.live_url) || defaults.live_url,
    live_status: bool(row.live_status, defaults.live_status),
...
async function fetchCreatorFromAnalyticsBridge(slug) {
  const rows = await supabaseFetch(
    "analytics_events?event_type=eq.creator_settings_" + normalizeSlug(slug) + "&select=metadata,created_at&order=created_at.desc&limit=1",
...
async function fetchCreator(slug) {
  ...
  return normalizeCreator(defaultCreator(safeSlug));
}
...
async function resolveLiveStatus(slug) {
  ...
  if (creator.auto_live_detection_enabled !== false) {
    ...
    try {
      ...
      if (runtimePayload && !runtimePayload.stale) {
        payload = runtimePayload;
      } else if (runtimePayload && runtimePayload.stale) {
        runtimeError = runtimePayload.error || "RUNTIME_STALE";
      }
    } catch (error) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const creator = normalizeCreator(defaultCreator(slug));
