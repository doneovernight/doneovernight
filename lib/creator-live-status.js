...
const { getSupabaseRuntimeConfig } = require("./creator-os-environment");
const CreatorCapabilities = require("./creator-capabilities");
...
const DEFAULT_MINA_SETTINGS = {
  ...
  battle_win_streak: 0,
  battle_updated_at: "",
  capabilities: CreatorCapabilities.normalizeCapabilities({
...
function normalizeUsername(value) {
  return clean(value || DEFAULT_CREATOR_SLUG).replace(/^@+/, "").toLowerCase().replace(/[^a-z0-9._-]/g, "") || DEFAULT_CREATOR_SLUG;
}
...
function normalizeCreator(row = {}) {
  ...
  const mirrored = CreatorCapabilities.mirrorLegacyFields({}, capabilities);
  return {
    ...DEFAULT_MINA_SETTINGS,
    ...row,
    capabilities,
    ...
    battle_mode_enabled: capabilities.battle === true && bool(row.battle_mode_enabled, false),
...
async function fetchCreatorFromTable(slug) {
  ...
  const fieldAttempts = [
    baseFields.join(","),
    baseFields.filter((field) => field !== "capabilities").join(","),
    baseFields.filter((field) => !["capabilities", "live_enabled", "battle_enabled"].includes(field)).join(",")
  ];
  let rows = null;
  let lastError = null;
  for (const fields of fieldAttempts) {
    try {
      rows = await supabaseFetch("creators?slug=eq." + encodeURIComponent(slug) + "&select=" + fields + "&limit=1", {
...
async function fetchCreatorFromAnalyticsBridge(slug = DEFAULT_CREATOR_SLUG) {
  const rows = await supabaseFetch(
    "analytics_events?event_type=eq.creator_settings_" + normalizeSlug(slug).replace(/[^a-z0-9_]+/g, "_") + "&select=metadata,created_at&order=created_at.desc&limit=25",
...
async function resolveLiveStatus(slug) {
  ...
  const username = normalizeUsername(creator.tiktok_live_username || creator.username);
  if (creator.capabilities && creator.capabilities.live === false) {
    return basePayload({
      isLive: false,
      username,
      creator: { ...creator, battle_mode_enabled: false },
      source: "disabled",
      capabilities: {}
    });
  }
