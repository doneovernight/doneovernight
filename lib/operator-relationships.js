...
async function claimOperatorClientRelationship({ portalRequest = {}, operatorSlug = "", source = "operator_referral" } = {}) {
  ...
  try {
    ...
    if (!existing?.id && profile?.email) {
      ...
      const telegramResult = await sendNewClientJoinedTelegram({
        operator,
        portalRequest,
        workspaceSlug,
        source,
        donReference
      }).catch((error) => ({ sent: false, status: error.message || "operator_telegram_failed" }));
      if (relationship?.id) {
        ...
        writeSupabasePayload(`operator_client_relationships?id=eq.${encodeURIComponent(relationship.id)}`, "PATCH", {
          raw_payload: {
            ...
            new_client_telegram_sent: telegramResult.sent === true,
            new_client_telegram_status: telegramResult.status,
            new_client_telegram_sent_at: telegramResult.sent ? now : null
          },
          updated_at: now
        }).catch(() => {});
      }
