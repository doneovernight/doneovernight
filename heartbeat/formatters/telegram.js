...
function formatHeartbeatTelegram(summary) {
  ...
  return [
    ...
    `Portal Review: ${statusLine(summary.health.portalReview)}`,
    `Admin: ${statusLine(summary.health.adminWebsite)}`,
    `Task API: ${statusLine(summary.health.taskApi)}`,
    `Task Requests: ${metricValue(summary.health.taskCount)}`,
    `Dispatch Signups: ${metricValue(summary.health.dispatchCount)}`,
    "",
    "Traffic",
